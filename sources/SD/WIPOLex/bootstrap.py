#!/usr/bin/env python3
"""
SD/WIPOLex -- Sudan legislation via the WIPO Lex database.
...
Republic of the Sudan it holds the full text of core statutes -- the acts and
regulations on patents, trademarks, copyright, industrial designs and related
commercial matters, plus broader instruments such as the Investment Encouragement
Act and the Customs Act -- as machine-readable PDF documents. Sudan's texts are
published in English and Arabic.

Sudan has no comprehensive open legislation API or bulk download reachable from
outside the country; what public collections exist (e.g. the gazette / national
assembly portals) are unreliable from datacenter IPs and serve scanned PDFs.
WIPO Lex publishes machine-readable Sudan statute text without authentication
and is reachable from datacenter IPs, making it a stable full-text source of the
country's law.

Strategy:
  1. Fetch the WIPO Lex Sudan member profile
     (/wipolex/en/members/profile/SD). It is server-rendered HTML listing each
  ...
  2. For each detail page, extract the CloudFront-signed PDF download URL(s)
     (https://wipolex-res.wipo.int/edocs/lexdocs/laws/{lang}/sd/{code}.pdf).
     The signed `?last-modified=...` query string is required -- the bare URL
     returns an HTML error page. English is preferred, then Arabic, then French.
...
logger = logging.getLogger("legal-data-hunter.SD.WIPOLex")

HOST = "https://www.wipo.int"
COUNTRY = "SD"
COUNTRY_NAME = "Sudan"
PROFILE_PATH = f"/wipolex/en/members/profile/{COUNTRY}"
SOURCE_ID = "SD/WIPOLex"
DELAY = 1.5
...
# .../edocs/lexdocs/laws/en/sd/sd001en.pdf
LANG_RE = re.compile(r"/edocs/lexdocs/laws/([a-z]{2})/[a-z]{2}/")

# Language preference: English first, then Arabic (Sudan publishes its
# legislation in both), then French.
LANG_PRIORITY = {"en": 0, "ar": 1, "fr": 2}
...
def _parse_profile(html_text: str) -> List[Tuple[str, str, Optional[str]]]:
    """Return [(detail_id, title, iso_date)] for every Sudan legal text.
...
class WIPOLexSDScraper(BaseScraper):
    """Scraper for SD/WIPOLex."""
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        ...
        for detail_id, title, date in self._iter_docs():
            ...
            doc_id = f"SD_WIPOLEX_{detail_id}"
            text = None
            chosen_url = None
            language = None
            for url in urls:
    ...
    def fetch_updates(self, since: Any = None) -> Generator[Dict[str, Any], None, None]:
        # WIPO Lex has no incremental feed; re-scan the (small) SD corpus.
    ...
    def test(self) -> bool:
        logger.info("Testing SD/WIPOLex profile + detail + PDF extraction...")
        profile = _get(f"{HOST}{PROFILE_PATH}")
        if not profile:
            logger.error("Profile fetch failed")
            return False
        rows = _parse_profile(profile)
        logger.info("Parsed %d SD legal texts", len(rows))
        if len(rows) < 10:
            logger.error("Expected >=10 SD texts, got %d", len(rows))
            return False
