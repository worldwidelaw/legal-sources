#!/usr/bin/env python3
"""
AF/WIPOLex -- Afghanistan legislation via the WIPO Lex database.

WIPO Lex (https://www.wipo.int/wipolex/) is WIPO's free, open gateway to the
intellectual-property and IP-related laws of ~200 jurisdictions. For
Afghanistan it holds the full text of the framework legal codes — the
Constitution, the Civil Code, the Penal Law, the Commercial / Customs / Press
laws and the core IP statutes — as official PDF documents in English, Pashto
and/or Dari (Farsi).

Afghanistan is one of the most data-poor jurisdictions in the project: every
native government legislation portal is either catalog-only (laws.af),
Cloudflare-gated (AsianLII / Stanford ALEP) or requires OCR of scanned gazette
PDFs (moj.gov.af). WIPO Lex is the one source that publishes machine-readable
full text and is reachable without authentication.

Strategy:
  1. Fetch the WIPO Lex Afghanistan member profile
     (/wipolex/en/members/profile/AF). It is server-rendered HTML listing every
     Afghan legal text as a table row: adoption date, title and a link to the
  ...
  2. For each detail page, extract the CloudFront-signed PDF download URL(s)
     (https://wipolex-res.wipo.int/edocs/lexdocs/laws/{lang}/af/{code}.pdf).
     Where several language versions exist, English is preferred, falling back
...
logger = logging.getLogger("legal-data-hunter.AF.WIPOLex")

HOST = "https://www.wipo.int"
PROFILE_PATH = "/wipolex/en/members/profile/AF"
SOURCE_ID = "AF/WIPOLex"
DELAY = 1.5
...
LANG_RE = re.compile(r"/edocs/lexdocs/laws/([a-z]{2})/af/")

# Language preference: English first, then Dari/Farsi, then Pashto, then any.
LANG_PRIORITY = {"en": 0, "fa": 1, "ps": 2}
LANG_NAMES = {"en": "English", "fa": "Dari/Farsi", "ps": "Pashto", "ar": "Arabic"}
...
def _parse_profile(html_text: str) -> List[Tuple[str, str, Optional[str]]]:
    """Return [(detail_id, title, iso_date)] for every Afghan legal text.
...
class WIPOLexAFScraper(BaseScraper):
    """Scraper for AF/WIPOLex."""
    ...
    def _iter_docs(self) -> Generator[Tuple[str, str, Optional[str]], None, None]:
        ...
        if not profile:
            logger.error("Could not fetch AF member profile")
            return
        rows = _parse_profile(profile)
        logger.info("AF member profile lists %d legal texts", len(rows))
        yield from rows
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        ...
        for detail_id, title, date in self._iter_docs():
            ...
            doc_id = f"AF_WIPOLEX_{detail_id}"
            text = None
            chosen_url = None
            language = None
            for url in urls:
    ...
    def fetch_updates(self, since: Any = None) -> Generator[Dict[str, Any], None, None]:
        # WIPO Lex has no incremental feed; re-scan the (small) AF corpus.
    ...
    def test(self) -> bool:
        logger.info("Testing AF/WIPOLex profile + detail + PDF extraction...")
        profile = _get(f"{HOST}{PROFILE_PATH}")
        if not profile:
            logger.error("Profile fetch failed")
            return False
        rows = _parse_profile(profile)
        logger.info("Parsed %d AF legal texts", len(rows))
        if len(rows) < 10:
            logger.error("Expected >=10 AF texts, got %d", len(rows))
            return False
