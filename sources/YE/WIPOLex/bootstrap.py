#!/usr/bin/env python3
"""
YE/WIPOLex -- Yemen legislation via the WIPO Lex database.

WIPO Lex (https://www.wipo.int/wipolex/) is WIPO's free, open gateway to the
intellectual-property and IP-related laws of ~200 jurisdictions. For Yemen it
holds the full text of core statutes -- the laws and codes on industrial
property, copyright, commerce, investment, customs and related acts -- as
machine-readable PDF documents, almost all in Arabic (Yemen's legislative
language), with a few in English.

Yemen has no comprehensive open legislation API or database reachable from
outside the country; years of conflict have left its official legal-publication
infrastructure offline or unreliable. WIPO Lex publishes machine-readable
Yemeni statute text without authentication, and is one of the very few stable
full-text sources of Yemeni law available externally.

Strategy:
  1. Fetch the WIPO Lex Yemen member profile
     (/wipolex/en/members/profile/YE). It is server-rendered HTML listing each
  ...
  2. For each detail page, extract the CloudFront-signed PDF download URL(s)
     (https://wipolex-res.wipo.int/edocs/lexdocs/laws/{lang}/ye/{code}.pdf).
     The signed `?last-modified=...` query string is required -- the bare URL
     returns an HTML error page. Arabic is preferred (Yemen's legislative
...
logger = logging.getLogger("legal-data-hunter.YE.WIPOLex")

HOST = "https://www.wipo.int"
COUNTRY = "YE"
COUNTRY_NAME = "Yemen"
PROFILE_PATH = f"/wipolex/en/members/profile/{COUNTRY}"
SOURCE_ID = "YE/WIPOLex"
DELAY = 1.5
...
# .../edocs/lexdocs/laws/ar/ye/ye001ar_1.pdf
LANG_RE = re.compile(r"/edocs/lexdocs/laws/([a-z]{2})/[a-z]{2}/")

# Language preference: Arabic first (Yemen's legislative language), then
# English, then French.
LANG_PRIORITY = {"ar": 0, "en": 1, "fr": 2}
...
def _parse_profile(html_text: str) -> List[Tuple[str, str, Optional[str]]]:
    """Return [(detail_id, title, iso_date)] for every Yemeni legal text.
...
class WIPOLexYEScraper(BaseScraper):
    """Scraper for YE/WIPOLex."""
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        ...
        for detail_id, title, date in self._iter_docs():
            ...
            doc_id = f"YE_WIPOLEX_{detail_id}"
            text = None
            chosen_url = None
            language = None
            for url in urls:
    ...
    def fetch_updates(self, since: Any = None) -> Generator[Dict[str, Any], None, None]:
        # WIPO Lex has no incremental feed; re-scan the (small) YE corpus.
    ...
    def test(self) -> bool:
        logger.info("Testing YE/WIPOLex profile + detail + PDF extraction...")
        profile = _get(f"{HOST}{PROFILE_PATH}")
        if not profile:
            logger.error("Profile fetch failed")
            return False
        rows = _parse_profile(profile)
        logger.info("Parsed %d YE legal texts", len(rows))
        if len(rows) < 10:
            logger.error("Expected >=10 YE texts, got %d", len(rows))
            return False
