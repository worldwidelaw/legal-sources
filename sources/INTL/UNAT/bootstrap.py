...
Fetches judgments from the United Nations Appeals Tribunal (UNAT).

Strategy:
  - Scrape yearly index pages (judgments_YYYY.shtml) for PDF links
  - Filter for UNAT judgment PDFs (pattern: YYYY-UNAT-NNNN.pdf)
  - Download each PDF and extract full text via common/pdf_extract

Data Coverage:
  - ~1500+ judgments from 2010 to present
  - Appellate body for UN staff employment disputes
...
logger = logging.getLogger("legal-data-hunter.INTL.UNAT")

BASE_URL = "https://www.un.org"
INDEX_URL = f"{BASE_URL}/en/internaljustice/unat/judgments-orders.shtml"
YEAR_URL_TEMPLATE = f"{BASE_URL}/en/internaljustice/unat/judgments_{{year}}.shtml"
MAX_PDF_BYTES = 50 * 1024 * 1024
START_YEAR = 2010


class UNATScraper(BaseScraper):
    """Scraper for UN Appeals Tribunal judgments."""
    ...
    def _get_judgment_pdfs(self, year: int) -> list[dict]:
        """Get all UNAT judgment PDF links for a given year."""
        ...
        # Find UNAT judgment PDFs — pattern: YYYY-UNAT-NNNN.pdf or YYYY-unat-NNN.pdf
        # Exclude translated variants (AR, FR suffixes) and outcome summaries
        pdf_pattern = r'href="([^"]*(?:unat|UNAT)[^"]*\.pdf)"'
        ...
        for pdf_path in matches:
            ...
            # Skip non-judgment files (outcome summaries, orders)
            if "outcome" in filename:
                continue
            # Skip translated versions (AR, FR, E suffixes)
            if re.search(r'-unat-\d+\s*(ar|fr|e)\.pdf$', filename, re.IGNORECASE):
                continue
            if re.search(r'unat-\d+[a-z]\.pdf$', filename, re.IGNORECASE):
                # e.g., 1516E.pdf — skip translated variants
                if not re.search(r'unat-\d+\.pdf$', filename, re.IGNORECASE):
                    continue
            ...
            num_match = re.search(r'(\d{4})-(?:unat|UNAT)-(\d+)', pdf_path, re.IGNORECASE)
            if num_match:
                j_year = num_match.group(1)
                j_num = num_match.group(2)
                judgment_id = f"UNAT/{j_year}/{j_num}"
            else:
                judgment_id = filename.replace(".pdf", "")

            entries.append({
                "judgment_id": judgment_id,
                "year": year,
                "pdf_url": full_url,
                "title": f"UNAT Judgment {judgment_id}",
            })
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        ...
        for year in years:
            ...
            logger.info(f"Year {year}: {len(entries)} UNAT judgments")
