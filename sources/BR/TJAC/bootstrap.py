#!/usr/bin/env python3
"""
BR/TJAC -- Acre State Court (Tribunal de Justiça do Estado do Acre)

Fetches court decisions (acordãos) from TJAC's eSAJ jurisprudence search.
~14k+ acordãos with ementas (legal summaries containing holdings and reasoning).

Endpoint: POST https://esaj.tjac.jus.br/cjsg/resultadoCompleta.do
...
logger = logging.getLogger("legal-data-hunter.BR.TJAC")
...
DELAY = 2.0
PAGE_SIZE = 20  # TJAC eSAJ returns 20 results per page
...
class TJACScraper(BaseScraper):
    """Scraper for BR/TJAC -- Acre State Court decisions via eSAJ."""
    ...
    def fetch_all(self, sample: bool = False) -> Generator[dict, None, None]:
        """Fetch all TJAC acordãos via eSAJ search, iterating by year."""
        self._init_session()
        count = 0
