...
Fetches Argentine National Labor Appeals Court decisions from SAIJ.

Data source: https://www.saij.gob.ar
License: Open public access (government data)

Strategy:
  - SAIJ search API with tribunal filter for CNAT
...
logger = logging.getLogger("legal-data-hunter.AR.CNAT")
...
TRIBUNAL_NAME = "CAMARA NACIONAL DE APELACIONES DEL TRABAJO"
...
def fetch_all(max_records: int = None, output_file: str = None) -> Generator[dict, None, None]:
    ...
    total = result.get("searchResults", {}).get("totalSearchResults", 0)
    logger.info(f"Total CNAT records: {total:,}")
...
def test_api():
    logger.info("Testing SAIJ API with CNAT tribunal filter...")
    try:
        result = search_documents(offset=0, page_size=2)
        total = result.get("searchResults", {}).get("totalSearchResults", 0)
        logger.info(f"Search API OK - Total CNAT results: {total:,}")
