#!/usr/bin/env python3
"""
PW/PacLII -- Palau Court Decisions & Legislation (PacLII) Fetcher

Fetches case law and legislation from the Pacific Islands Legal Information
Institute (PacLII) for Palau.

Strategy:
  - Case law: iterate Supreme Court database by year, collect document links
  - Legislation: iterate alpha-indexed listing pages, collect document links
  - Court rules: iterate alpha-indexed listing pages
...
logger = logging.getLogger("legal-data-hunter.PW.PacLII")

BASE_URL = "https://www.paclii.org"

COURTS = [
    {"code": "PWSC", "name": "Supreme Court", "path": "/pw/cases/PWSC/"},
]

LEGIS_DATABASES = [
    {"type": "consol_act", "name": "Consolidated Legislation", "path": "/pw/legis/consol_act/"},
    {"type": "num_act", "name": "Sessional Legislation", "path": "/pw/legis/num_act/"},
]

RULES_DATABASES = [
    {"type": "ct_rules", "name": "Court Rules", "path": "/pw/rules/ct_rules/"},
    {"type": "prof_conduct_rules", "name": "Professional Conduct Rules", "path": "/pw/rules/prof_conduct_rules/"},
]
...
class PalauPacLIIScraper(BaseScraper):
    """Scraper for PW/PacLII -- Palau legal data."""
    ...
    def _extract_document(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        Some Palau decisions have inline HTML text; others are PDF-only.
        Try inline first; if too short, fall back to PDF extraction.
        """
        ...
        # PacLII PDFs require a Referer header, so download with our session
        if len(text) < 500:
            pdf_link = None
            for a in body.find_all("a", href=True):
                if a["href"].lower().endswith(".pdf"):
                    ...
                    break
            # Also check for "alt : X.pdf" pattern (PDF-only stub pages)
            if not pdf_link:
        ...
        # 500 chars minimum: PDF-only stub pages have ~350 chars of nav boilerplate
        if len(text) < 500:
            return None
    ...
    def _parse_case_date(self, title: str) -> Optional[str]:
        """Extract date from case title like '[2024] PWSC 3 (29 January 2024)'."""
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        """Fetch all Palau documents from PacLII."""
        count = 0
    ...
    def test(self) -> bool:
        ...
        resp = self._request(BASE_URL + "/pw/cases/PWSC/")
        if resp is None:
            logger.error("Cannot reach PacLII Palau Supreme Court")
            return False

        logger.info("PacLII Palau Supreme Court page OK")

        doc = self._extract_document(BASE_URL + "/pw/cases/PWSC/2026/1.html")
        if doc:
            logger.info(f"Case OK: {doc['title'][:60]} ({len(doc['text'])} chars)")
        else:
            doc = self._extract_document(BASE_URL + "/pw/cases/PWSC/2025/1.html")
            if doc:
                logger.info(f"Case OK: {doc['title'][:60]} ({len(doc['text'])} chars)")
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="PW/PacLII data fetcher")
    parser.add_argument(
        "command",
