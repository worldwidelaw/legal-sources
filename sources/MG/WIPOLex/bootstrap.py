#!/usr/bin/env python3
"""
MG/WIPOLex -- Madagascar legislation via the WIPO Lex database.

WIPO Lex (https://www.wipo.int/wipolex/) is WIPO's free, open gateway to the
intellectual-property and IP-related laws of ~200 jurisdictions. For Madagascar
it holds the full text of core statutes -- not only IP law but broader
...
documents, predominantly in French (Madagascar's main legislative language
alongside Malagasy).

Madagascar has no comprehensive open legislation API or bulk download reachable
from outside the country; the official gazette (Journal Officiel de la
Republique de Madagascar) is blocked or unreliable from datacenter IPs. WIPO Lex
...
Strategy:
  1. Fetch the WIPO Lex Madagascar member profile
     (/wipolex/en/members/profile/MG). It is server-rendered HTML listing each
  ...
  2. For each detail page, extract the CloudFront-signed PDF download URL(s)
     (https://wipolex-res.wipo.int/edocs/lexdocs/laws/{lang}/mg/{code}.pdf).
     The signed `?last-modified=...` query string is required -- the bare URL
     returns an HTML error page. French is preferred (Madagascar's main
     legislative language), then English.
...
# .../edocs/lexdocs/laws/fr/mg/mg001fr.pdf
LANG_RE = re.compile(r"/edocs/lexdocs/laws/([a-z]{2})/[a-z]{2}/")

# Language preference: French first (Madagascar's main legislative language),
# then English.
...
def _parse_profile(html_text: str) -> List[Tuple[str, str, Optional[str]]]:
    """Return [(detail_id, title, iso_date)] for every Madagascar legal text.
...
class WIPOLexMGScraper(BaseScraper):
    """Scraper for MG/WIPOLex."""
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        ...
        for detail_id, title, date in self._iter_docs():
            ...
            doc_id = f"MG_WIPOLEX_{detail_id}"
            text = None
            chosen_url = None
            language = None
            for url in urls:
    ...
    def test(self) -> bool:
        logger.info("Testing MG/WIPOLex profile + detail + PDF extraction...")
        profile = _get(f"{HOST}{PROFILE_PATH}")
        if not profile:
            logger.error("Profile fetch failed")
            return False
        rows = _parse_profile(profile)
        logger.info("Parsed %d MG legal texts", len(rows))
        if len(rows) < 10:
            logger.error("Expected >=10 MG texts, got %d", len(rows))
            return False
