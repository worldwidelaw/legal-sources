#!/usr/bin/env python3
"""
BF/WIPOLex -- Burkina Faso legislation via the WIPO Lex database.

WIPO Lex (https://www.wipo.int/wipolex/) is WIPO's free, open gateway to the
intellectual-property and IP-related laws of ~200 jurisdictions. For Burkina
Faso it holds the full text of core statutes -- not only IP law but broader
instruments such as the codes and framework laws on industrial property,
copyright, investment and related commercial matters -- as machine-readable PDF
documents, predominantly in French (Burkina Faso's legislative language).

Burkina Faso has no comprehensive open legislation API or bulk download
reachable from outside the country; the official gazette (Journal Officiel du
Burkina Faso) and the national legislation portal (LegiBurkina) are blocked or
unreliable from datacenter IPs. WIPO Lex publishes machine-readable Burkina Faso
statute text without authentication and is reachable from datacenter IPs, making
it a stable full-text source of the country's law.

Strategy:
  1. Fetch the WIPO Lex Burkina Faso member profile
     (/wipolex/en/members/profile/BF). It is server-rendered HTML listing each
  ...
  2. For each detail page, extract the CloudFront-signed PDF download URL(s)
     (https://wipolex-res.wipo.int/edocs/lexdocs/laws/{lang}/bf/{code}.pdf).
     The signed `?last-modified=...` query string is required -- the bare URL
     returns an HTML error page. French is preferred (Burkina Faso's legislative
...
# .../edocs/lexdocs/laws/fr/bf/bf001fr.pdf
LANG_RE = re.compile(r"/edocs/lexdocs/laws/([a-z]{2})/[a-z]{2}/")

# Language preference: French first (Burkina Faso's legislative language), then
# English.
LANG_PRIORITY = {"fr": 0, "en": 1, "ar": 2}
...
def _parse_profile(html_text: str) -> List[Tuple[str, str, Optional[str]]]:
    """Return [(detail_id, title, iso_date)] for every Burkina Faso legal text.
...
def _detail_pdf_urls(detail_id: str) -> List[str]:
    """Signed PDF download URLs for a detail page, French first."""
...
class WIPOLexBFScraper(BaseScraper):
    """Scraper for BF/WIPOLex."""
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        ...
        for detail_id, title, date in self._iter_docs():
            ...
            doc_id = f"BF_WIPOLEX_{detail_id}"
            text = None
            chosen_url = None
            language = None
            for url in urls:
    ...
    def fetch_updates(self, since: Any = None) -> Generator[Dict[str, Any], None, None]:
        # WIPO Lex has no incremental feed; re-scan the (small) BF corpus.
    ...
    def test(self) -> bool:
        logger.info("Testing BF/WIPOLex profile + detail + PDF extraction...")
        profile = _get(f"{HOST}{PROFILE_PATH}")
        if not profile:
            logger.error("Profile fetch failed")
            return False
        rows = _parse_profile(profile)
        logger.info("Parsed %d BF legal texts", len(rows))
        if len(rows) < 10:
            logger.error("Expected >=10 BF texts, got %d", len(rows))
            return False
