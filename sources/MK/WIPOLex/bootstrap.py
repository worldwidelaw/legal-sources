#!/usr/bin/env python3
"""
MK/WIPOLex -- North Macedonia legislation via the WIPO Lex database.

WIPO Lex (https://www.wipo.int/wipolex/) is WIPO's free, open gateway to the
intellectual-property and IP-related laws of ~200 jurisdictions. For North
Macedonia it holds the full text of core statutes — the Constitution, the Penal
Code, the Code of Criminal Procedure, the Customs Code, the Law on Copyright and
Related Rights, the Law on Industrial Property and a range of related acts — as
machine-readable PDF documents, available in both English (WIPO translations)
and Macedonian.

North Macedonia's native legislation channels are blocked from outside the
country:

- **Sluzben Vesnik** (slvesnik.com.mk) — the Official Gazette portal is blocked
- **Sobranie** (sobranie.mk) — the Assembly law database is blocked
...
Strategy:
  1. Fetch the WIPO Lex North Macedonia member profile
     (/wipolex/en/members/profile/MK). It is server-rendered HTML listing each
  ...
  2. For each detail page, extract the CloudFront-signed PDF download URL(s)
     (https://wipolex-res.wipo.int/edocs/lexdocs/laws/{lang}/mk/{code}.pdf).
     The signed `?last-modified=...` query string is required — the bare URL
     returns an HTML error page. English is preferred, Macedonian as fallback.
...
# .../edocs/lexdocs/laws/en/mk/mk014en.pdf
LANG_RE = re.compile(r"/edocs/lexdocs/laws/([a-z]{2})/[a-z]{2}/")

# Language preference: English WIPO translation first, Macedonian original next.
LANG_PRIORITY = {"en": 0, "mk": 1}
LANG_NAMES = {"en": "English", "mk": "Macedonian", "fr": "French"}
...
def _parse_profile(html_text: str) -> List[Tuple[str, str, Optional[str]]]:
    """Return [(detail_id, title, iso_date)] for every North Macedonia legal text.
...
class WIPOLexMKScraper(BaseScraper):
    """Scraper for MK/WIPOLex."""
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        ...
        for detail_id, title, date in self._iter_docs():
            ...
            doc_id = f"MK_WIPOLEX_{detail_id}"
            text = None
            chosen_url = None
            language = None
            for url in urls:
    ...
    def fetch_updates(self, since: Any = None) -> Generator[Dict[str, Any], None, None]:
        # WIPO Lex has no incremental feed; re-scan the (small) MK corpus.
    ...
    def test(self) -> bool:
        logger.info("Testing MK/WIPOLex profile + detail + PDF extraction...")
        profile = _get(f"{HOST}{PROFILE_PATH}")
        if not profile:
            logger.error("Profile fetch failed")
            return False
        rows = _parse_profile(profile)
        logger.info("Parsed %d MK legal texts", len(rows))
        if len(rows) < 10:
            logger.error("Expected >=10 MK texts, got %d", len(rows))
            return False
