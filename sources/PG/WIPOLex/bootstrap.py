#!/usr/bin/env python3
"""
PG/WIPOLex -- Papua New Guinea legislation via the WIPO Lex database.

WIPO Lex (https://www.wipo.int/wipolex/) is WIPO's free, open gateway to the
intellectual-property and IP-related laws of ~200 jurisdictions. For Papua New
Guinea it holds the full text of core statutes -- not only IP law but broader
...
documents, in English (Papua New Guinea's legislative language).

Papua New Guinea has no comprehensive open legislation API or bulk download
reachable from outside the country; the consolidated legislation database (Office
of Legislative Counsel) and the National Gazette are blocked or unreliable from
datacenter IPs, and the regional LII (PacLII) blocks datacenter IPs. WIPO Lex
publishes machine-readable Papua New Guinea statute text without authentication
and is reachable from datacenter IPs, making it a stable full-text source of the
country's law.

Strategy:
  1. Fetch the WIPO Lex Papua New Guinea member profile
     (/wipolex/en/members/profile/PG). It is server-rendered HTML listing each
  ...
  2. For each detail page, extract the CloudFront-signed PDF download URL(s)
     (https://wipolex-res.wipo.int/edocs/lexdocs/laws/{lang}/pg/{code}.pdf).
     The signed `?last-modified=...` query string is required -- the bare URL
     returns an HTML error page. English is preferred (Papua New Guinea's
     legislative language), then French.
...
# .../edocs/lexdocs/laws/en/pg/pg001en.pdf
LANG_RE = re.compile(r"/edocs/lexdocs/laws/([a-z]{2})/[a-z]{2}/")

# Language preference: English first (Papua New Guinea's legislative language),
# then French.
LANG_PRIORITY = {"en": 0, "fr": 1, "ar": 2}
...
def _parse_profile(html_text: str) -> List[Tuple[str, str, Optional[str]]]:
    """Return [(detail_id, title, iso_date)] for every Papua New Guinea legal text.
...
class WIPOLexPGScraper(BaseScraper):
    """Scraper for PG/WIPOLex."""
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        ...
        for detail_id, title, date in self._iter_docs():
            ...
            doc_id = f"PG_WIPOLEX_{detail_id}"
            text = None
            chosen_url = None
            language = None
            for url in urls:
    ...
    def fetch_updates(self, since: Any = None) -> Generator[Dict[str, Any], None, None]:
        # WIPO Lex has no incremental feed; re-scan the (small) PG corpus.
    ...
    def test(self) -> bool:
        logger.info("Testing PG/WIPOLex profile + detail + PDF extraction...")
        profile = _get(f"{HOST}{PROFILE_PATH}")
        if not profile:
            logger.error("Profile fetch failed")
            return False
        rows = _parse_profile(profile)
        logger.info("Parsed %d PG legal texts", len(rows))
        if len(rows) < 10:
            logger.error("Expected >=10 PG texts, got %d", len(rows))
            return False
