#!/usr/bin/env python3
"""
SM/WIPOLex -- San Marino legislation via the WIPO Lex database.
...
of San Marino it holds the full text of core statutes -- the consolidation law on
industrial property, the copyright law, and numerous council decrees ratifying
international IP and related instruments -- as machine-readable PDF documents in
Italian (San Marino's legislative language).

San Marino has domestic legislation portals (Bollettino Ufficiale, Legisammarino)
but no open bulk download or API reachable from outside the microstate. WIPO Lex
publishes machine-readable San Marino statute text without authentication and is
a stable external full-text source of the country's law.

Strategy:
  1. Fetch the WIPO Lex San Marino member profile
     (/wipolex/en/members/profile/SM). It is server-rendered HTML listing each
  ...
  2. For each detail page, extract the CloudFront-signed PDF download URL(s)
     (https://wipolex-res.wipo.int/edocs/lexdocs/laws/{lang}/sm/{code}.pdf).
     The signed `?last-modified=...` query string is required -- the bare URL
     returns an HTML error page. Italian is preferred (San Marino's legislative
     language), then English, then French.
...
# .../edocs/lexdocs/laws/it/sm/sm001it.pdf
LANG_RE = re.compile(r"/edocs/lexdocs/laws/([a-z]{2})/[a-z]{2}/")

# Language preference: Italian first (San Marino's legislative language), then
# English, then French.
LANG_PRIORITY = {"it": 0, "en": 1, "fr": 2}
...
def _parse_profile(html_text: str) -> List[Tuple[str, str, Optional[str]]]:
    """Return [(detail_id, title, iso_date)] for every San Marino legal text.
...
def _detail_pdf_urls(detail_id: str) -> List[str]:
    """Signed PDF download URLs for a detail page, Italian first."""
...
class WIPOLexSMScraper(BaseScraper):
    """Scraper for SM/WIPOLex."""
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        ...
        for detail_id, title, date in self._iter_docs():
            ...
            doc_id = f"SM_WIPOLEX_{detail_id}"
            text = None
            chosen_url = None
            language = None
            for url in urls:
    ...
    def fetch_updates(self, since: Any = None) -> Generator[Dict[str, Any], None, None]:
        # WIPO Lex has no incremental feed; re-scan the (small) SM corpus.
    ...
    def test(self) -> bool:
        logger.info("Testing SM/WIPOLex profile + detail + PDF extraction...")
        profile = _get(f"{HOST}{PROFILE_PATH}")
        if not profile:
            logger.error("Profile fetch failed")
            return False
        rows = _parse_profile(profile)
        logger.info("Parsed %d SM legal texts", len(rows))
        if len(rows) < 10:
            logger.error("Expected >=10 SM texts, got %d", len(rows))
            return False
