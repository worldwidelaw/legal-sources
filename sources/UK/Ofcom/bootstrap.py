#!/usr/bin/env python3
"""
Legal Data Hunter - UK Ofcom Scraper

Fetches Ofcom regulatory documents via:
  - GOV.UK Search API (paginated discovery of all Ofcom publications)
  - GOV.UK Content API (full text for each document)

Coverage: ~370 documents (reports, guidance, statistics, consultations)
...
logger = logging.getLogger("UK/Ofcom")
...
class UKOfcomScraper(BaseScraper):
    """
    Scraper for UK Ofcom publications on GOV.UK.

    Strategy:
    - Use GOV.UK Search API to list all Ofcom documents (paginated)
    ...
    def _search_page(self, start: int = 0) -> dict:
        """Fetch one page of Ofcom documents from GOV.UK Search API."""
        params = {
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield all Ofcom documents from GOV.UK."""
        start = 0
        total = None
        fetched = 0
        ...
        while True:
            ...
            if total is None:
                ...
                logger.info(f"Total Ofcom documents on GOV.UK: {total}")
