...
sanctions enforcement, and transparency documents.

NCA's own site (nationalcrimeagency.gov.uk) is blocked by Azure WAF,
so we use the GOV.UK mirror for accessible content.
...
logger = logging.getLogger("UK/NCA")

BASE_URL = "https://www.gov.uk"
ORG_SLUG = "national-crime-agency"
...
class UKNCAScraper(BaseScraper):
    """
    Scraper for UK NCA documents via GOV.UK Content API.

    Strategy:
    - Search API with organisation filter for NCA content
    ...
    def _search_documents(self, start: int = 0, count: int = 200) -> dict:
        """Search GOV.UK for NCA documents."""
        params = (
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield all NCA documents with full text."""
        start = 0
        while True:
            result = self._search_documents(start=start)
            total = result.get("total", 0)
            items = result.get("results", [])

            if start == 0:
                logger.info(f"Total NCA documents: {total}")
