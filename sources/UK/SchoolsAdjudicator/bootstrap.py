#!/usr/bin/env python3
"""
Legal Data Hunter - UK Office of the Schools Adjudicator (OSA) Decisions Scraper

Fetches determinations of the Office of the Schools Adjudicator (OSA) from
GOV.UK using:
  - GET /api/search.json?filter_format=decision&filter_organisations=office-of-the-schools-adjudicator (discovery)
...
under the School Standards and Framework Act 1998. Adjudicators rule on
objections to the admission arrangements of maintained schools and academies
in England, on variations to those arrangements, and on disputes about school
organisation (e.g. transfer of land). Their determinations are binding on the
admission authority concerned.

Coverage: ~2,240 determinations. On GOV.UK the `decision` format entries for
this body carry a short summary in details.body and the full determination as
a PDF attachment (assets.publishing.service.gov.uk) — born-digital, extracted
via the shared PDF extractor (no OCR needed).
...
logger = logging.getLogger("UK/SchoolsAdjudicator")

# Case reference like ADA4602 / VAR1234 / STP0987 in title or determination body.
_REF_RE = re.compile(r"\b(ADA|VAR|STP|REF)\s?\d{3,5}\b", re.IGNORECASE)


class UKSchoolsAdjudicatorScraper(BaseScraper):
    """Scraper for the Office of the Schools Adjudicator determinations on GOV.UK."""

    BASE_URL = "https://www.gov.uk"
    SEARCH_URL = "/api/search.json"
    CONTENT_URL = "/api/content"
    FILTER_FORMAT = "decision"
    FILTER_ORG = "office-of-the-schools-adjudicator"
    PAGE_SIZE = 200
    ...
    def _extract_pdf_text(self, content_id: str, url: str) -> str:
        ...
        try:
            text = pdf_extract.extract_pdf_markdown(
                "UK/SchoolsAdjudicator", content_id, pdf_url=url,
            )
        except Exception as e:
    ...
    def _build_raw(self, item: dict, content: dict, text: str, pdf_url: str) -> dict:
        ...
        ref = ""
        m = _REF_RE.search(title) or _REF_RE.search(text[:2000])
        if m:
    ...
    def normalize(self, raw: dict) -> dict:
        ...
        return {
            "_id": f"UK/SchoolsAdjudicator/{raw.get('content_id', '')}",
            "_source": "UK/SchoolsAdjudicator",
            "_type": "case_law",
...
if __name__ == "__main__":
    scraper = UKSchoolsAdjudicatorScraper()
