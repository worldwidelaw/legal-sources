#!/usr/bin/env python3
"""
Legal Data Hunter - UK Ofwat Scraper

Fetches Ofwat regulatory documents via:
  - GOV.UK Search API (paginated discovery of all Ofwat publications)
  - GOV.UK Content API (full text for each document)

Coverage: ~61 documents (policy papers, corporate reports, guidance, consultations)
...
logger = logging.getLogger("UK/Ofwat")

SEARCH_URL = "https://www.gov.uk/api/search.json"
CONTENT_URL = "https://www.gov.uk/api/content"
ORG_FILTER = "the-water-services-regulation-authority"
PAGE_SIZE = 50
...
class UKOfwatScraper(BaseScraper):
    """
    Scraper for UK Ofwat publications on GOV.UK.

    Strategy:
    - Use GOV.UK Search API to list all Ofwat documents (paginated)
    ...
    def _search_page(self, start: int = 0) -> dict:
        """Fetch one page of Ofwat documents from GOV.UK Search API."""
        params = {
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield all Ofwat documents from GOV.UK."""
        start = 0
        total = None
        fetched = 0
        ...
        while True:
            ...
            if total is None:
                ...
                logger.info(f"Total Ofwat documents on GOV.UK: {total}")
            ...
            for item in results:
                ...
                if doc_type in ("finder", "finder_email_signup", "organisation", "official_statistics_announcement"):
                    continue
