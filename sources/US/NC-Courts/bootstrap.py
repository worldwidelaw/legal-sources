#!/usr/bin/env python3
"""
US/NC-Courts -- North Carolina Appellate Court Opinions

Fetches NC Supreme Court and Court of Appeals opinions with full text via
CourtListener's free search API and PDF storage.

Strategy:
  1. Search CourtListener API for NC opinions (no auth needed)
...
Data: Public domain (NC government works). No auth required.
...
logger = logging.getLogger("legal-data-hunter.US.NC-Courts")

SEARCH_URL = "https://www.courtlistener.com/api/rest/v4/search/"
STORAGE_URL = "https://storage.courtlistener.com"

# NC courts on CourtListener
NC_COURTS = {
...
class NCCourtsScraper(BaseScraper):
    ...
    def test_api(self):
        ...
        logger.info("Testing CourtListener search API for NC courts...")
        try:
            data = self._get_json(
                f"{SEARCH_URL}?type=o&court=nc&order_by=dateFiled+desc&page_size=1"
            )
            count = data.get("count", 0)
            results = data.get("results", [])
            if count > 0 and results:
                case = results[0]
                logger.info(f"  NC Supreme Court opinions: {count}")
    ...
    def search_opinions(self, court_ids: str = "nc,ncctapp",
                        page_size: int = 20, cursor: str = None,
                        filed_after: str = None) -> dict:
        """Search CourtListener for NC opinions."""
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield all NC appellate court opinions."""
        total = 0
        cursor = None
    ...
    def fetch_sample(self) -> Generator[dict, None, None]:
        """Fetch a sample of recent NC opinions from both courts."""
        logger.info("Fetching sample opinions...")
        count = 0

        for court_id, court_name in NC_COURTS.items():
            logger.info(f"  Sampling from {court_name}...")
            data = self.search_opinions(court_ids=court_id, page_size=8)
            results = data.get("results", [])
...
def main():
    ...
    parser = argparse.ArgumentParser(description="US/NC-Courts bootstrap")
    parser.add_argument(
        "command",
