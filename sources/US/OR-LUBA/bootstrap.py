...
Fetches the full text of every Final Opinion and Order of the Oregon Land
Use Board of Appeals (LUBA), Oregon's specialized quasi-judicial tribunal
with exclusive jurisdiction to review local government land-use decisions
(comprehensive plan and zoning amendments, conditional-use permits,
subdivisions, urban-growth-boundary and goal-compliance disputes, etc.)
under ORS 197.805–197.860. Each Final Opinion and Order resolves a specific
appeal = case_law. These are official Oregon state-government works in the
public domain (government edicts).

The corpus is published by the Oregon State Library / LUBA as a CONTENTdm
digital collection (alias ``p17027coll12``, "Oregon Land Use Board of Appeals
Final Orders and Opinions") on the OCLC-hosted CONTENTdm instance at
``https://cdm17027.contentdm.oclc.org/``. CONTENTdm exposes a public,
un-authenticated JSON web-services API (``dmwebservices``):

  - Enumerate:  dmQuery/p17027coll12/0/dmrecord!date/dmrecord/{max}/{start}/0/0/0/0/json
                -> pager.total + records[].dmrecord (each an item pointer)
  - Per item:   dmGetItemInfo/p17027coll12/{dmrecord}/json
                -> all Dublin-Core fields, including:
                     title   = LUBA case number    (e.g. "2023-071")
                     subjec  = Case name (parties)
...
logger = logging.getLogger("legal-data-hunter.US.OR-LUBA")
...
class ORLUBAScraper(BaseScraper):
    ...
    def discover_pointers(self, sample: bool = False) -> Generator[str, None, None]:
        ...
        while True:
            ...
            if sample or start >= total or len(recs) < page:
                break
        logger.info(f"Discovered {len(seen)} LUBA order pointers")
    ...
    def _build_raw(self, rec: str) -> dict | None:
        ...
        order_type = self._field(info.get("descri"))
        title_bits = []
        if case_number:
            title_bits.append(f"LUBA No. {case_number}")
        if case_name:
            title_bits.append(case_name)
        title = " — ".join(title_bits) or f"LUBA Order {rec}"
        return {
            "rec": rec,
            "case_number": case_number,
            "case_name": case_name,
            "order_type": order_type,
            "disposition": self._field(info.get("type")),
            "author": self._field(info.get("creato")),
            "title": title,
            "text": text,
    ...
    def test_api(self) -> bool:
        logger.info("Testing Oregon LUBA CONTENTdm API...")
        try:
    ...
    def normalize(self, raw: dict) -> dict:
        return {
            ...
            "issuer": "Oregon Land Use Board of Appeals (LUBA)",
            "title": raw["title"],
            "text": raw["text"],
            "url": raw["url"],
...
def main():
    ...
    parser = argparse.ArgumentParser(description="US/OR-LUBA bootstrap")
    parser.add_argument(
        "command",
