#!/usr/bin/env python3
"""
US/OH-Courts -- Ohio Supreme Court & Appellate Decisions

Fetches Ohio court opinions via CourtListener search API + PDF full text
from storage.courtlistener.com.

Courts covered:
  - ohio: Ohio Supreme Court (~87,800 opinions)
  - ohioctapp: Ohio Court of Appeals (~109,300 opinions)

Data access:
  - Search API: https://www.courtlistener.com/api/rest/v4/search/
  - PDFs: https://storage.courtlistener.com/{local_path}
...
logger = logging.getLogger("legal-data-hunter.US.OH-Courts")

SEARCH_URL = "https://www.courtlistener.com/api/rest/v4/search/"
PDF_BASE = "https://storage.courtlistener.com/"
COURTS = ["ohio", "ohioctapp"]
DELAY = 2.0
PAGE_SIZE = 20
...
def fetch_pdf_text(session: requests.Session, local_path: str) -> str:
    url = PDF_BASE + local_path
    try:
...
def court_name(court_id: str) -> str:
    ...
    }.get(court_id, f"Ohio Court ({court_id})")


def normalize(raw: Dict[str, Any]) -> Dict[str, Any]:
    cluster_id = raw.get("cluster_id", "")
    court_id = raw.get("court_id", "ohio")
    _id = f"US-OH-{court_id}-{cluster_id}"
    ...
    for court in COURTS:
        ...
        for page in range(1, max_pages_per_court + 1):
            ...
            for result in data["results"]:
                ...
                # Find an opinion with a local_path for PDF download
                local_path = None
                for op in opinions:
                ...
                if not local_path:
                    logger.debug(f"  No PDF path for {result.get('caseName', 'unknown')}")
                    continue

                time.sleep(DELAY)
                logger.info(f"  Fetching PDF: {result.get('caseName', '')[:50]}")
                text = fetch_pdf_text(session, local_path)
...
def test_connectivity() -> bool:
    session = get_session()
    try:
        data = search_court(session, "ohio", 1)
        if not data:
            return False
        count = data.get("count", 0)
        ...
        if results and results[0].get("opinions"):
            ...
            if lp:
                text = fetch_pdf_text(session, lp)
                logger.info(f"PDF test: {len(text)} chars")
                return len(text) > 0
        return True
