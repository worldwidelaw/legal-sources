#!/usr/bin/env python3
"""
US/MN-Courts -- Minnesota Supreme Court & Court of Appeals
...
Data Coverage:
  - Minnesota Supreme Court: ~45,000 opinions
  - Minnesota Court of Appeals: ~13,000 opinions
  - Language: English
...
logger = logging.getLogger("legal-data-hunter.US.MN-Courts")
...
class MNCourtsScraper(BaseScraper):

    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield all Minnesota court opinions from both courts."""
    ...
    def normalize(self, raw: dict) -> Optional[dict]:
        ...
        doc_id = f"mn-courts-{opinion_id}"

        return {
            "_id": doc_id,
            "_source": "US/MN-Courts",
            "_type": "case_law",
...
def main():
    ...
    scraper = MNCourtsScraper(str(source_dir))
    ...
    if command == "test":
        print("Testing CourtListener search API for Minnesota courts...")
        try:
            resp = SESSION.get(API_BASE, params={
                "q": "*", "court": "minn", "type": "o", "page_size": 1
            }, timeout=15)
            resp.raise_for_status()
            data = resp.json()
            print(f"  MN Supreme Court: {data['count']} opinions")

            resp2 = SESSION.get(API_BASE, params={
                "q": "*", "court": "minnctapp", "type": "o", "page_size": 1
            }, timeout=15)
            resp2.raise_for_status()
            data2 = resp2.json()
            print(f"  MN Court of Appeals: {data2['count']} opinions")
