#!/usr/bin/env python3
"""
US/MN-Courts -- Minnesota Supreme Court
...
from common.courtlistener import OpinionKeyedCourtListenerScraper, run_cli


class MNCourtsScraper(OpinionKeyedCourtListenerScraper):
    SOURCE_ID = 'US/MN-Courts'
    COURT_ORDER = ['minn', 'minnctapp']
    COURT_NAMES = {
        'minn': 'Minnesota Supreme Court',
        'minnctapp': 'Minnesota Court of Appeals',
    }
    # One row per opinion, keyed f"{OPINION_ID_PREFIX}-{opinion_id}" —
    # the legacy granularity and dedup key, preserved so the rows already
    # indexed under it are not orphaned (issue #1495).
    OPINION_ID_PREFIX = 'mn-courts'
    JURISDICTION = 'US-MN'


if __name__ == "__main__":
    run_cli(MNCourtsScraper)
