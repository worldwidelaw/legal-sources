...
Fetches the full text of selected orders of the Oregon Court of Appeals
("Orders of Interest") -- orders resolving specific motions and matters in
appeals before the court (e.g. orders allowing or denying petitions,
dismissals, attorney-fee awards, and other case dispositions that do not
issue as full written opinions). Each order resolves a specific contested
matter in a named case = case_law. These are official Oregon state-government
judicial works in the public domain (government edicts).

The corpus is published by the Oregon State Library / Oregon Judicial
Department as a CONTENTdm digital collection (alias ``p17027coll17``,
"Oregon Court of Appeals Orders of Interest") on the OCLC-hosted CONTENTdm
instance at ``https://cdm17027.contentdm.oclc.org/``. CONTENTdm exposes a
public, un-authenticated JSON web-services API (``dmwebservices``):

  - Enumerate:  dmQuery/p17027coll17/0/dmrecord!date/dmrecord/{max}/{start}/0/0/0/0/json
                -> pager.total + records[].dmrecord (each an item pointer)
  - Per item:   dmGetItemInfo/p17027coll17/{dmrecord}/json
                -> Dublin-Core fields:
                     title   = Case Title
                     identi  = Case Number
                     subjec  = Case Classification
                     date    = Date of Order (Entry Date, ISO YYYY-MM-DD)
                     descri  = Allowed or Denied
...
logger = logging.getLogger("legal-data-hunter.US.OR-CourtOfAppealsOrders")
...
class ORCourtOfAppealsOrdersScraper(BaseScraper):
    ...
    def discover_pointers(self, sample: bool = False) -> Generator[str, None, None]:
        ...
        while True:
            ...
            if sample or start >= total or len(recs) < page:
                break
        logger.info(f"Discovered {len(seen)} Court of Appeals order pointers")
    ...
    def _build_raw(self, rec: str) -> dict | None:
        ...
        if not text or len(text) < 200:
            logger.warning(f"No usable transcript for rec {rec} "
                           f"({len(text)} chars)")
            return None
        case_title = self._field(info.get("title"))
        case_number = self._field(info.get("identi"))
        title = case_title or f"Oregon Court of Appeals Order {rec}"
        if case_number and case_title:
            title = f"{case_title} ({case_number})"
        return {
            "rec": rec,
            "case_title": case_title,
            "case_number": case_number,
            "classification": self._field(info.get("subjec")),
            "disposition": self._field(info.get("descri")),
            "order_type": self._field(info.get("creato")),
            "issued_by": self._field(info.get("publis")),
            "title": title[:300],
            "text": text,
    ...
    def test_api(self) -> bool:
        logger.info("Testing Oregon Court of Appeals Orders CONTENTdm API...")
        try:
            ...
            if raw and raw["text"] and len(raw["text"]) > 200:
                logger.info(f"  Text extraction OK ({len(raw['text'])} chars) — "
                            f"{raw.get('case_title')} [{raw.get('date')}]")
            else:
    ...
    def normalize(self, raw: dict) -> dict:
        return {
            "_id": f"US/OR-CourtOfAppealsOrders/{raw['rec']}",
            "_source": "US/OR-CourtOfAppealsOrders",
            "_type": "case_law",
...
def main():
    ...
    parser = argparse.ArgumentParser(description="US/OR-CourtOfAppealsOrders bootstrap")
    parser.add_argument(
        "command",
    ...
    scraper = ORCourtOfAppealsOrdersScraper()
