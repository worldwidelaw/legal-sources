...
Fetches the full text of every published Board Order of the Oregon
Employment Relations Board (ERB), the state's quasi-judicial agency that
adjudicates public-sector labor-relations disputes -- unfair labor practice
complaints, representation / unit-clarification cases, contract dispute and
declaratory-ruling matters, and appeals from the State Personnel Relations
Board -- under Oregon's Public Employee Collective Bargaining Act (PECBA,
ORS 243) and related statutes. Each Board Order resolves a specific
contested case = case_law. These are official Oregon state-government works
in the public domain (government edicts).

The corpus is published by the Oregon State Library / ERB as a CONTENTdm
digital collection (alias ``p17027coll9``, "Employment Relations Board
Orders") on the OCLC-hosted CONTENTdm instance at
``https://cdm17027.contentdm.oclc.org/``. CONTENTdm exposes a public,
un-authenticated JSON web-services API (``dmwebservices``):

  - Enumerate:  dmQuery/p17027coll9/0/dmrecord!date/dmrecord/{max}/{start}/0/0/0/0/json
                -> pager.total + records[].dmrecord (each an item pointer)
  - Per item:   dmGetItemInfo/p17027coll9/{dmrecord}/json
...
The ``transc`` field carries the FULL decision text (tens of thousands of
characters per order), so no PDF download / extraction is required.
...
logger = logging.getLogger("legal-data-hunter.US.OR-ERB")
...
class ORERBScraper(BaseScraper):
    ...
    def discover_pointers(self, sample: bool = False) -> Generator[str, None, None]:
        ...
        while True:
            ...
            if sample or start >= total or len(recs) < page:
                break
        logger.info(f"Discovered {len(seen)} ERB order pointers")
    ...
    def _build_raw(self, rec: str) -> dict | None:
        ...
        if not text or len(text) < 200:
            logger.warning(f"No usable transcript for rec {rec} "
                           f"({len(text)} chars)")
            return None
        case_number = self._field(info.get("title"))
        case_name = self._field(info.get("subjec"))
        title = case_number or f"ERB Order {rec}"
        if case_name:
            title = f"{title} — {case_name}" if case_number else case_name
        return {
            "rec": rec,
            "case_number": case_number,
            "case_name": case_name,
            "case_type": self._field(info.get("type")),
            "title": title,
            "text": text,
    ...
    def test_api(self) -> bool:
        logger.info("Testing Oregon ERB CONTENTdm API...")
        try:
    ...
    def normalize(self, raw: dict) -> dict:
        return {
            ...
            "issuer": "Oregon Employment Relations Board (ERB)",
            "title": raw["title"],
            "text": raw["text"],
            "url": raw["url"],
...
def main():
    ...
    parser = argparse.ArgumentParser(description="US/OR-ERB bootstrap")
    parser.add_argument(
        "command",
