#!/usr/bin/env python3
"""
US/TN-Courts -- Tennessee Supreme Court
...
class TNCourtsScraper(CourtListenerScraper):
    SOURCE_ID = 'US/TN-Courts'
    COURT_ORDER = ['tenn', 'tennctapp', 'tenncrimapp']
    COURT_NAMES = {
    ...
    def doc_id_for(self, court_id, cluster_id):
        # Legacy key: the cluster id alone, with no court segment.
        # Preserved verbatim so already-indexed rows keep their dedup key.
        return f"tn-{cluster_id}"


if __name__ == "__main__":
    run_cli(TNCourtsScraper)
