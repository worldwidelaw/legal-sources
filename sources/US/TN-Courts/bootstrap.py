#!/usr/bin/env python3
"""
US/TN-Courts -- Tennessee Appellate Court Opinions

Fetches Tennessee Supreme Court, Court of Appeals, and Court of Criminal
Appeals opinions with full text via CourtListener's free search API and
PDF storage.

Strategy:
  1. Search CourtListener API for TN opinions (no auth needed)
...
Data: Public domain (TN government works). No auth required.
...
logger = logging.getLogger("legal-data-hunter.US.TN-Courts")
...
class TNCourtsScraper(BaseScraper):
    ...
    def _get_json(self, url: str) -> dict:
        """Fetch URL with rate limiting, return JSON."""
        time.sleep(self.delay)
        resp = self.http.get(url)
        return resp.json()

    def _get_bytes(self, url: str) -> bytes:
        """Fetch URL with rate limiting, return raw bytes."""
        time.sleep(self.delay)
        resp = self.http.get(url)
        return resp.content
    ...
    def test_api(self):
        ...
        logger.info("Testing CourtListener search API for TN courts...")
        try:
            data = self._get_json(
                f"{SEARCH_URL}?type=o&court=tenn&order_by=dateFiled+desc&page_size=1"
            )
            count = data.get("count", 0)
            results = data.get("results", [])
            if count > 0 and results:
                case = results[0]
                logger.info(f"  TN Supreme Court opinions: {count}")
                logger.info(f"  Latest case: {case['caseName']} ({case['dateFiled']})")
    ...
    def fetch_opinion_text(self, result: dict) -> Optional[str]:
        ...
        # Fall back to snippet if PDF fails
        for opinion in opinions:
    ...
    def normalize(self, result: dict, text: str) -> dict:
        """Transform search result + extracted text into standard schema."""
        now = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
        cluster_id = result.get("cluster_id", 0)

        return {
            "_id": f"tn-{cluster_id}",
            "_source": "US/TN-Courts",
            "_type": "case_law",
            "_fetched_at": now,
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield all TN appellate court opinions."""
        total = 0
        cursor = None
        ...
        while True:
            ...
            for result in results:
                ...
                if text and len(text) > 50:
                    ...
                    logger.warning(f"Skipping {result.get('caseName')}: no text extracted")
    ...
    def fetch_sample(self) -> Generator[dict, None, None]:
        """Fetch a sample of recent TN opinions from all three courts."""
        logger.info("Fetching sample opinions...")
        count = 0

        for court_id, court_name in TN_COURTS.items():
            logger.info(f"  Sampling from {court_name}...")
            data = self.search_opinions(court_ids=court_id, page_size=6)
            results = data.get("results", [])
...
def main():
    ...
    parser = argparse.ArgumentParser(description="US/TN-Courts bootstrap")
    parser.add_argument(
        "command",
