...
Strategy:
  ...
  2. For each title, crawl chapter -> subchapter -> part hierarchy
  3. At the leaf level, extract individual section links
...
class NYAdminCodeScraper(BaseScraper):
    ...
    def discover_hierarchy(self, page_path: str) -> list:
        """Discover child links from a hierarchy page.

        Returns a list of paths. Detects whether children are:
        - Chapters, subchapters, parts, subparts (hierarchy levels)
        - Sections (/regulations/new-york/N-NYCRR-...) (leaf level)
        """
        ...
        sections = []
        for m in re.finditer(r'href="(/regulations/new-york/(\d+)-NYCRR-[^"]+)"', html):
            path = m.group(1)
            if '#' not in path:
        ...
        # Look for hierarchy links
        children = []
        for level in ['chapter', 'subchapter', 'part', 'subpart',
                       'division', 'article', 'subtitle']:
            pattern = r'href="(' + re.escape(page_path) + r'/' + level + r'-[^"]+)"'
            found = self._extract_links(html, pattern)
            if found:
                children.extend(found)

        # Fallback: any deeper path links
        if not children:
    ...
    def crawl_to_sections(self, page_path: str, depth: int = 0) -> list:
        ...
        # Check if children are section links (N-NYCRR-...)
        if children and re.match(r'/regulations/new-york/\d+-NYCRR-', children[0]):
            return children
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Fetch all NYCRR sections with full text."""
    ...
    def fetch_updates(self, since: str) -> Generator[dict, None, None]:
        """Fetch updates since a date. NYCRR doesn't support date filtering."""
