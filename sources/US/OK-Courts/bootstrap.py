#!/usr/bin/env python3
"""
US/OK-Courts -- Oklahoma Supreme Court
...
class OKCourtsScraper(CourtListenerScraper):
    SOURCE_ID = 'US/OK-Courts'
    COURT_ORDER = ['okla', 'oklacivapp', 'oklacrimapp']
    COURT_NAMES = {
    ...
    def doc_id_for(self, court_id, cluster_id):
        # Legacy key: the raw CourtListener court id, not an abbreviation.
        # Preserved verbatim so already-indexed rows keep their dedup key.
        return f"{self.ID_PREFIX}-{court_id or 'okla'}-{cluster_id}"


if __name__ == "__main__":
    run_cli(OKCourtsScraper)
