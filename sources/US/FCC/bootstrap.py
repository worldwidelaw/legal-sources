#!/usr/bin/env python3
"""
US/FCC -- Federal Communications Commission Orders and Rulings

Fetches FCC regulatory documents (rules, proposed rules, notices) from the
Federal Register API. Full text is retrieved from the Federal Register's
plain-text endpoint for each document.

Coverage: ~10,000+ FCC documents from 1994 to present.
...
logger = logging.getLogger("legal-data-hunter.US.FCC")
...
def search_documents(session: requests.Session, doc_type: str,
    ...
    """Search Federal Register for FCC documents of a given type."""
    params = {
...
def fetch_all(sample: bool = False) -> Generator[Dict[str, Any], None, None]:
    """Fetch all FCC documents from the Federal Register API."""
    session = get_session()
    total_yielded = 0
    sample_limit = 15
