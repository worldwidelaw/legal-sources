#!/usr/bin/env python3
"""
US/FederalReserve -- Federal Reserve Board Enforcement Actions and Regulatory Guidance

Fetches Federal Reserve regulatory documents (rules, proposed rules, notices) from the
Federal Register API. Full text is retrieved from the Federal Register's
plain-text endpoint for each document.

Coverage: ~10,000+ Federal Reserve documents from 1994 to present.
...
logger = logging.getLogger("legal-data-hunter.US.FederalReserve")
...
def search_documents(session: requests.Session, doc_type: str,
    ...
    """Search Federal Register for Federal Reserve documents of a given type."""
    params = {
...
def normalize(doc: Dict, full_text: str) -> Dict[str, Any]:
    ...
    return {
        "_id": f"US-FedReserve-{doc_number}",
        "_source": "US/FederalReserve",
        "_type": "doctrine",
...
def fetch_all(sample: bool = False) -> Generator[Dict[str, Any], None, None]:
    """Fetch all Federal Reserve documents from the Federal Register API."""
    session = get_session()
    total_yielded = 0
    sample_limit = 15
