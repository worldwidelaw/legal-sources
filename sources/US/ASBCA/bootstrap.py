#!/usr/bin/env python3
"""
US/ASBCA -- Armed Services Board of Contract Appeals

Fetches ASBCA decisions with full text via CourtListener's free search
API and PDF storage.

Strategy:
  1. Search CourtListener API for ASBCA decisions (no auth needed)
...
logger = logging.getLogger("legal-data-hunter.US.ASBCA")
...
class ASBCAScraper(BaseScraper):
    ...
    def test_api(self):
        ...
        logger.info("Testing CourtListener search API for ASBCA...")
        try:
            data = self._get_json(
                f"{SEARCH_URL}?type=o&court=asbca&order_by=dateFiled+desc&page_size=1"
            )
            count = data.get("count", 0)
            results = data.get("results", [])
            if count > 0 and results:
                case = results[0]
                logger.info(f"  ASBCA decisions: {count}")
    ...
    def search_decisions(self, page_size: int = 20, cursor: str = None,
        ...
        url = f"{SEARCH_URL}?type=o&court=asbca&order_by=dateFiled+desc&page_size={page_size}"
        if cursor:
            url = cursor
    ...
    def fetch_sample(self) -> Generator[dict, None, None]:
        logger.info("Fetching sample ASBCA decisions...")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="US/ASBCA bootstrap")
    parser.add_argument(
        "command",
