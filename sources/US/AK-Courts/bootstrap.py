#!/usr/bin/env python3
"""
US/AK-Courts -- Alaska Supreme Court & Court of Appeals Opinions
...
Courts covered:
  - Supreme Court of Alaska (alaska) — ~7K opinions from 1960
  - Court of Appeals of Alaska (alaskactapp) — ~3.7K opinions from 1980
...
class AKCourtsScraper(BaseScraper):
    """
    Scraper for US/AK-Courts — Alaska Supreme Court & Court of Appeals.
    ...
    def normalize(self, raw: Dict[str, Any]) -> Dict[str, Any]:
        ...
        court_abbr = "AKSC" if court_id == "alaska" else "AKCA"
        doc_id = f"US-AK-{court_abbr}-{cluster_id}"

        court_name = COURT_NAMES.get(court_id, raw.get("court", "Alaska Court"))

        return {
            "_id": doc_id,
            "_source": "US/AK-Courts",
            "_type": "case_law",
    ...
    def test_connection(self) -> bool:
        try:
            ...
            results = data.get("results", [])
            logger.info(f"Connection test: {count} total AK opinions, got {len(results)} results")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="US/AK-Courts data fetcher")
    parser.add_argument(
        "command",
    ...
    parser.add_argument("--full", action="store_true", help="Full bootstrap (all pages)")
    args = parser.parse_args()

    scraper = AKCourtsScraper()
