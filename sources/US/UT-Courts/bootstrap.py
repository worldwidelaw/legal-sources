#!/usr/bin/env python3
"""
US/UT-Courts -- Utah Supreme Court & Court of Appeals Opinions

Fetches Utah appellate opinions with full text via CourtListener's free
search API and PDF storage.

Strategy:
  1. Search CourtListener API for Utah opinions (no auth needed)
...
logger = logging.getLogger("legal-data-hunter.US.UT-Courts")
...
class UTCourtsScraper(BaseScraper):
    ...
    def test_api(self):
        ...
        logger.info("Testing CourtListener search API for UT courts...")
        try:
            data = self._get_json(
                f"{SEARCH_URL}?type=o&court=utah&order_by=dateFiled+desc&page_size=1"
            )
            count = data.get("count", 0)
            results = data.get("results", [])
            if count > 0 and results:
                case = results[0]
                logger.info(f"  Utah Supreme Court opinions: {count}")
    ...
    def search_opinions(self, court_ids: str = "utah,utahctapp",
    ...
    def fetch_sample(self) -> Generator[dict, None, None]:
        logger.info("Fetching sample Utah court opinions...")
        count = 0
        for court_id, court_name in UT_COURTS.items():
...
def main():
    ...
    parser = argparse.ArgumentParser(description="US/UT-Courts bootstrap")
    parser.add_argument(
        "command",
