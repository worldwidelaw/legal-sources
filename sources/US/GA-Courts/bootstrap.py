#!/usr/bin/env python3
"""
US/GA-Courts -- Georgia Supreme Court & Court of Appeals
...
Data Coverage:
  - Georgia Supreme Court: ~73,000 opinions (1841-present)
  - Georgia Court of Appeals: ~132,000 opinions (1907-present)
  - Language: English
...
logger = logging.getLogger("legal-data-hunter.US.GA-Courts")
...
class GACourtsScraper(BaseScraper):

    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield all Georgia court opinions from both courts."""
        ...
        while url:
            ...
            try:
                if page == 1:
                    ...
                    # Subsequent pages use the cursor URL directly
    ...
    def normalize(self, raw: dict) -> Optional[dict]:
        ...
        doc_id = f"ga-courts-{opinion_id}"

        return {
            "_id": doc_id,
            "_source": "US/GA-Courts",
            "_type": "case_law",
...
def main():
    ...
    scraper = GACourtsScraper(str(source_dir))
    ...
    if command == "test":
        print("Testing CourtListener search API...")
        try:
            resp = SESSION.get(API_BASE, params={
                "q": "*", "court": "ga", "type": "o", "page_size": 1
            }, timeout=15)
            resp.raise_for_status()
            data = resp.json()
            print(f"  GA Supreme Court: {data['count']} opinions")

            resp2 = SESSION.get(API_BASE, params={
                "q": "*", "court": "gactapp", "type": "o", "page_size": 1
            }, timeout=15)
            resp2.raise_for_status()
            data2 = resp2.json()
            print(f"  GA Court of Appeals: {data2['count']} opinions")
