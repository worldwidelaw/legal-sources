#!/usr/bin/env python3
"""
GA/WIPOLex -- Gabon legislation via the WIPO Lex database.
...
of Gabon it holds the full text of core statutes -- the acts and regulations on
...
PDF documents in French (Gabon's legislative language).

Gabon has no comprehensive open legislation API, bulk download or free LII-style
platform reachable from outside the country; what exists (the Journal Officiel and
ministry portals) is either offline-only, paywalled or unreliable from datacenter
IPs. WIPO Lex publishes machine-readable Gabon statute text without authentication
and is a stable, commercially-usable full-text source of the country's law.

Strategy:
  1. Fetch the WIPO Lex Gabon member profile
     (/wipolex/en/members/profile/GA). It is server-rendered HTML listing each
  ...
  2. For each detail page, extract the CloudFront-signed PDF download URL(s)
     (https://wipolex-res.wipo.int/edocs/lexdocs/laws/{lang}/ga/{code}.pdf).
     The signed `?last-modified=...` query string is required -- the bare URL
     returns an HTML error page. French is preferred (Gabon's legislative
     language).
...
logger = logging.getLogger("legal-data-hunter.GA.WIPOLex")

HOST = "https://www.wipo.int"
COUNTRY = "GA"
COUNTRY_NAME = "Gabon"
PROFILE_PATH = f"/wipolex/en/members/profile/{COUNTRY}"
SOURCE_ID = "GA/WIPOLex"
DELAY = 1.5
...
# .../edocs/lexdocs/laws/fr/ga/ga001fr.pdf
LANG_RE = re.compile(r"/edocs/lexdocs/laws/([a-z]{2})/[a-z]{2}/")

# Language preference: French first (Gabon's legislative language), then English.
LANG_PRIORITY = {"fr": 0, "en": 1}
...
def _parse_profile(html_text: str) -> List[Tuple[str, str, Optional[str]]]:
    """Return [(detail_id, title, iso_date)] for every Gabon legal text.
...
class WIPOLexGAScraper(BaseScraper):
    """Scraper for GA/WIPOLex."""
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        ...
        for detail_id, title, date in self._iter_docs():
            ...
            doc_id = f"GA_WIPOLEX_{detail_id}"
            text = None
            chosen_url = None
            language = None
            for url in urls:
    ...
    def fetch_updates(self, since: Any = None) -> Generator[Dict[str, Any], None, None]:
        # WIPO Lex has no incremental feed; re-scan the (small) GA corpus.
    ...
    def test(self) -> bool:
        logger.info("Testing GA/WIPOLex profile + detail + PDF extraction...")
        profile = _get(f"{HOST}{PROFILE_PATH}")
        if not profile:
            logger.error("Profile fetch failed")
            return False
        rows = _parse_profile(profile)
        logger.info("Parsed %d GA legal texts", len(rows))
        if len(rows) < 10:
            logger.error("Expected >=10 GA texts, got %d", len(rows))
            return False
