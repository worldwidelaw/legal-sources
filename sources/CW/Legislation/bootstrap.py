#!/usr/bin/env python3
"""
CW/Legislation -- Curacao Government Legislation (Regelingen)

Fetches Curacao legislation from the Dutch CVDR (Centraal Voorziening Decentrale
Regelgeving) via SRU 1.2 API. Covers landsverordeningen, ministerial regulations,
and landsbesluiten.

Strategy:
  1. SRU search with koninkrijksdeel="Curaçao" to get metadata + XML URLs
...
logger = logging.getLogger("legal-data-hunter.CW.Legislation")

SOURCE_ID = "CW/Legislation"
JURISDICTION_QUERY = 'koninkrijksdeel="Curaçao"'
...
class CWLegislationScraper(BaseScraper):
    """Scraper for CW/Legislation -- Curacao legislation via CVDR SRU."""
    ...
    def _download_full_text(self, xml_url: str) -> str:
        ...
        # Extract text from CVDR XML — target <al> (alinea) elements primarily
        # plus structural headings. Avoid extracting <label>/<nr> separately
        # since they duplicate heading text.
        parts = []
        for elem in root.iter():
            ...
            if tag_lower in ('al', 'intitule', 'citeertitel', 'tussenkop'):
                ...
                # Extract kop as a single line (label + nr + titel combined)
...
if __name__ == "__main__":
    ...
    if command == "test":
        ...
        print(f"Connection OK. Curacao CVDR records: {num.text if num is not None else '?'}")
        records = root.findall(".//srw:record", NS)
        if records:
