#!/usr/bin/env python3
"""
German Federal Administrative Court (BVerwG) Case Law Fetcher

Official open data from rechtsprechung-im-internet.de
https://www.rechtsprechung-im-internet.de

This fetcher retrieves case law from the Federal Administrative Court using:
- Table of Contents XML for document discovery (~10,000+ decisions)
...
RSS_FEED_URL = f"{BASE_URL}/jportal/docs/feed/bsjrs-bverwg.xml"
TOC_URL = f"{BASE_URL}/rii-toc.xml"
COURT_PREFIX = "BVerwG"


class BVerwGFetcher:
    """Fetcher for German Federal Administrative Court case law"""
    ...
    def _get_entries_from_toc(self, limit: int = None) -> List[Dict[str, Any]]:
        """Fetch BVerwG entries from the table of contents XML"""
        ...
        logger.info(f"Found {len(entries)} BVerwG entries in TOC")
        return entries

    def _get_entries_from_rss(self) -> List[Dict[str, Any]]:
        """Fetch BVerwG entries from RSS feed (for recent decisions)"""
        logger.info(f"Fetching RSS feed from {RSS_FEED_URL}")
        try:
            response = self.session.get(RSS_FEED_URL, timeout=60)
            response.raise_for_status()
        except requests.RequestException as e:
            logger.warning(f"RSS feed unavailable: {e}, falling back to TOC")
            return self._get_entries_from_toc(limit=50)
    ...
    def _parse_decision_xml(self, xml_content: str) -> Dict[str, Any]:
        ...
        if gruende_elem is not None:
            result['entscheidungsgruende'] = self._extract_text_from_element(gruende_elem)
    ...
    def fetch_all(self, limit: int = None, use_rss: bool = False) -> Iterator[Dict[str, Any]]:
        """
        Fetch BVerwG decisions with full text.

        Args:
            limit: Maximum number of decisions to fetch (None for all)
            use_rss: If True, use RSS feed (recent only). If False, use full TOC.

        Yields:
            Raw document dictionaries with full text
        """
        if use_rss:
            entries = self._get_entries_from_rss()
        else:
            entries = self._get_entries_from_toc(limit=limit)
    ...
    def normalize(self, raw_doc: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'court': raw_doc.get('gertyp', 'BVerwG'),
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'bootstrap':
        ...
        sample_count = 0
        target_count = 100 if '--sample' in sys.argv else 100

        # Use TOC for full discovery (10,000+ decisions)
        for raw_doc in fetcher.fetch_all(limit=target_count + 10, use_rss=False):
