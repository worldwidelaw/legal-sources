#!/usr/bin/env python3
"""
DE/ThueringenCaseLaw - Thüringen State Court Decisions

Fetches court decisions from the official Thüringen Landesrecht portal via the
juris jPortal REST API.

Coverage:
- ~5K court decisions from all Thüringen court branches
- VG, OVG, LG, OLG, AG, LAG, LSG, SG, FG, and constitutional court
- Decisions from 2010 onward

Data source: https://landesrecht.thueringen.de/bsth/
...
SOURCE_ID = "DE/ThueringenCaseLaw"
...
class JPortalSession:
    ...
    def search(self, start: int = 1, size: int = PAGE_SIZE) -> Optional[Dict]:
        ...
        body = {
            "searchTasks": {
                "RESULT_LIST": {"size": size, "start": start, "sort": "date"},
            },
            "filters": {
                "CATEGORY": [CATEGORY],
            },
...
def normalize(doc_id: str, doc_data: Dict, search_item: Dict = None) -> Dict:
    ...
    # Extract court and case number from title
    # Title format: "Aktenzeichen | Gericht | Datum | Entscheidungsart"
    ...
    # Fallback: parse from title parts
...
def fetch_all(limit: int = None) -> Iterator[Dict]:
    ...
    if total_hits == 0:
        total_hits = 5100  # fallback estimate

    # Paginate through all search results and collect doc IDs
    doc_ids = []
    start = 1
    consecutive_empty = 0
...
def fetch_sample(count: int = 15) -> List[Dict]:
    ...
    pages_to_try = [1, 500, 1000, 2000, 3000, 4000, 5000]
...
def main():
    parser = argparse.ArgumentParser(description="DE/ThueringenCaseLaw data fetcher")
    parser.add_argument(
        "command",
