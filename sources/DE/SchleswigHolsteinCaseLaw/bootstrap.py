#!/usr/bin/env python3
"""
DE/SchleswigHolsteinCaseLaw - Schleswig-Holstein State Court Decisions

Fetches court decisions from the official Schleswig-Holstein Landesrecht portal via the
juris jPortal REST API.

Coverage:
- ~9K court decisions from all Schleswig-Holstein court branches
- VG, OVG, LG, OLG, AG, LAG, LSG, SG, FG, and constitutional court
- Decisions from 2000s onward

Data source: https://gesetze-rechtsprechung.sh.juris.de/bssh/
...
class JPortalSession:
    ...
    def _api_headers(self) -> Dict[str, str]:
        return {
            ...
            "Origin": BASE_URL,
            "Referer": f"{BASE_URL}/{PORTAL_ID}/search",
        }
    ...
    def search(self, start: int = 1, size: int = PAGE_SIZE) -> Optional[Dict]:
        ...
        try:
            resp = self.session.post(f"{API_BASE}/search", json=body, headers=self._api_headers(), timeout=60)
            if resp.status_code != 200:
    ...
    def search_with_category_hits(self) -> Optional[Dict]:
        time.sleep(RATE_LIMIT_DELAY)
        body = {
            "searchTasks": {"CATEGORY_HITS": {}, "RESULT_LIST": {"size": 1, "start": 1}},
            "filters": {"CATEGORY": [CATEGORY]},
            "searches": [],
            "clientID": PORTAL_ID,
        }
    ...
    def fetch_document(self, doc_id: str, doc_part: str = "L", retries: int = 3) -> Optional[Dict]:
        body = {
            "docId": doc_id,
            "format": "xsl",
            "keyword": None,
        ...
        for attempt in range(retries):
            time.sleep(RATE_LIMIT_DELAY)
            try:
                resp = self.session.post(f"{API_BASE}/document", json=body, headers=self._api_headers(), timeout=120)
                if resp.status_code == 200:
                    return resp.json()
