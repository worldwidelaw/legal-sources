#!/usr/bin/env python3
"""
CL/BancoCentral -- Banco Central de Chile Normativa Fetcher

Fetches Central Bank of Chile regulations from the LeyChile platform,
filtered by organismo=Banco Central.

Data source: https://www.bcentral.cl/areas/normativa
Access via: https://nuevo.leychile.cl/servicios (BCN internal JSON API)
License: Open government data (Chile)

Strategy:
  - Use internal JSON API at nuevo.leychile.cl/servicios
  - Search with organismo=Banco+Central filter (~3,400 norms)
...
logger = logging.getLogger("legal-data-hunter.CL.BancoCentral")
...
def search_norms(page: int = 1, per_page: int = 20) -> tuple:
    """Search for BCCh norms via the buscarjson endpoint."""
    params = {
        'string': '',
        'organismo': ORGANISMO,
        'tipoNorma': '0',
...
def fetch_sample(count: int = 15) -> list:
    ...
    logger.info(f"Searching for BCCh norms (organismo={ORGANISMO})...")
    search_results, total = search_norms(page=1, per_page=count * 3)
    logger.info(f"Total BCCh norms available: {total:,}")
...
def test_api():
    ...
    logger.info("Testing BCCh norms via LeyChile API...")

    try:
        results, total = search_norms(page=1, per_page=3)
        logger.info(f"Search API OK - {total:,} BCCh norms, got {len(results)} results")
    except Exception as e:
