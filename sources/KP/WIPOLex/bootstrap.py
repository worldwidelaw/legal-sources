#!/usr/bin/env python3
"""
KP/WIPOLex -- North Korea (DPRK) legislation via the WIPO Lex database.
...
Democratic People's Republic of Korea it holds the full text of core statutes —
the Intellectual Property Law, the Laws on Inventions, Industrial Designs,
Trademarks and Copyright, and related acts — as machine-readable PDF documents
...
effectively the only source publishing machine-readable DPRK statute text
without authentication, which is why it is the project's first KP source.

Strategy:
  1. Fetch the WIPO Lex DPRK member profile
     (/wipolex/en/members/profile/KP). It is server-rendered HTML listing each
  ...
  2. For each detail page, extract the CloudFront-signed PDF download URL(s)
     (https://wipolex-res.wipo.int/edocs/lexdocs/laws/{lang}/kp/{code}.pdf).
     The signed `?last-modified=...` query string is required — the bare URL
     returns an HTML error page. English is preferred, Korean used as fallback.
...
logger = logging.getLogger("legal-data-hunter.KP.WIPOLex")

HOST = "https://www.wipo.int"
COUNTRY = "KP"
COUNTRY_NAME = "North Korea (DPRK)"
PROFILE_PATH = f"/wipolex/en/members/profile/{COUNTRY}"
SOURCE_ID = "KP/WIPOLex"
DELAY = 1.5
...
# Language preference: English first, Korean fallback.
LANG_PRIORITY = {"en": 0, "ko": 1}
...
class WIPOLexKPScraper(BaseScraper):
    """Scraper for KP/WIPOLex."""
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        ...
        for detail_id, title, date in self._iter_docs():
            ...
            doc_id = f"KP_WIPOLEX_{detail_id}"
            text = None
            chosen_url = None
            language = None
            for url in urls:
    ...
    def test(self) -> bool:
        logger.info("Testing KP/WIPOLex profile + detail + PDF extraction...")
        profile = _get(f"{HOST}{PROFILE_PATH}")
        if not profile:
            logger.error("Profile fetch failed")
            return False
        rows = _parse_profile(profile)
        logger.info("Parsed %d KP legal texts", len(rows))
        if len(rows) < 10:
            logger.error("Expected >=10 KP texts, got %d", len(rows))
            return False
