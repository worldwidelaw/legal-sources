#!/usr/bin/env python3
"""
EU/CouncilResolutions — Resolutions and declarations of the Council of the EU
(CELEX **sector 3**, document class ``G``).

Council **resolutions** are non-binding acts by which the Council (often together
with the Representatives of the Governments of the Member States) expresses a
political commitment or invites action in an area of Union policy. They are
published in the Official Journal and catalogued by the EU Publications Office
(CELLAR / EUR-Lex) under the sector-3 document descriptor ``G`` (CELEX
``3{YYYY}G{NNNN}``) — e.g. the Council Resolution on the EU Work Plan for Culture,
on the European Education Area, on a joint European degree label, and on the
...
variants) and therefore never pulls Council resolutions. C-series resolutions
carried under the ``Y`` descriptor (EU/OJC-Acts) are a *different* set of CELEX
works — no ``_id`` collision — so ``G`` is not otherwise captured.
...
1. Enumerate every resolution via the public CELLAR SPARQL endpoint (CELEX
   ...
   OFFSET ceiling, so a single paged LIMIT/OFFSET sweep suffices.
2. Fetch the full text of each act from CELLAR via HTTP content negotiation:
   OJ/Formex **xHTML** for modern resolutions, an **OJ HTML** manifestation via
   the language-suffixed CELEX (``.ENG``) for older ones, and a born-digital
   **PDF** stream (PyMuPDF) for the remainder. All are served by CELLAR, which
...
Same CELLAR recipe as EU/OJC-Acts and EU/CommissionOpinions, narrowed to the
sector-3 ``G`` Council-resolution descriptor. Corrigenda (``…R(01)``) and
...
# CELEX pattern for sector-3 Council resolutions: 3{YYYY}G{NNNN}[(NN)].
CELEX_RE = r"^3[0-9]{4}G[0-9]"
...
class CouncilResolutionsScraper(BaseScraper):
    """Scraper for EU Council resolutions (CELEX sector-3 ``G``) via CELLAR."""
    ...
    def normalize(self, raw: dict) -> dict:
        ...
        if not title:
            title = f"Council Resolution {celex}"
        date = raw.get("date")
        if date and not re.match(r"^\d{4}-\d{2}-\d{2}", str(date)):
            date = None
        return {
            "_id": celex,
            "_source": "EU/CouncilResolutions",
            "_type": "doctrine",
...
def main():
    ...
    scraper = CouncilResolutionsScraper()
    ...
    if command in ("bootstrap", "bootstrap-fast"):
        ...
        print(f"SPARQL returned {len(page)} Council resolutions on page 1")
        if page:
            c = page[0]["celex"]
