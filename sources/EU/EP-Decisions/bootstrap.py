#!/usr/bin/env python3
"""
EU/EP-Decisions — European Parliament decisions (sector-5 CELEX ``DP`` class).

Beyond its legislative positions (EU/EP-Positions, ``AP``) and non-legislative
resolutions (EU/EP-Resolutions, ``IP``), the European Parliament adopts a
distinct class of **decisions** in plenary: appointments and renewals of members
of EU bodies (Court of Auditors, ECB Executive Board, EU agencies), decisions on
requests for the waiver or defence of a Member's parliamentary immunity,
decisions not to object to Commission delegated/implementing acts, discharge and
own-resources decisions, and similar procedural acts. These carry a
``P{n}_TA(YYYY)NNNN`` texts-adopted identifier and are published in the OJ C
series, registered in the EU Publications Office repository (CELLAR) with a
CELEX number of the form ``5{YYYY}DP{NNNN}`` (sector 5 = preparatory acts;
"DP" = the EP-decision document class).

This is a distinct, additive companion to EU/EP-Positions and EU/EP-Resolutions
and to EU/EuroParl (whose adopted-texts API only reaches back to 2014); the DP
series spans 1999-present, so its pre-2014 half is cleanly additive (the loader
dedups on the CELEX ``_id``).

Data flow
---------
1. Enumerate every EP decision via the public CELLAR SPARQL endpoint (CELEX
   matching ``^5[0-9]{4}DP``), retrieving CELEX + document date + English
   expression title. Paged with LIMIT/OFFSET (the DP corpus is well under the
   ~10 000-row SPARQL OFFSET ceiling).
2. Fetch the full text of each decision from CELLAR via HTTP content
   negotiation:
...
Same CELLAR recipe as EU/EESC-Opinions, EU/CoR-Opinions and EU/EP-Resolutions,
narrowed to the "DP" descriptor.
"""
...
# CELEX pattern for EP decisions: 5{YYYY}DP{NNNN}
CELEX_RE = r"^5[0-9]{4}DP"
...
class EPDecisionsScraper(BaseScraper):
    """Scraper for European Parliament decisions (DP) via CELLAR."""
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield RAW decision dicts (with full text) for every EP decision."""
        offset = 0
        seen = set()
        while True:
    ...
    def fetch_updates(self, since: datetime) -> Generator[dict, None, None]:
        """Yield decisions with a document date on/after ``since`` (newest-first)."""
    ...
    def normalize(self, raw: dict) -> dict:
        ...
        if not title:
            title = f"European Parliament decision {celex}"
        date = raw.get("date")
        if date and not re.match(r"^\d{4}-\d{2}-\d{2}", str(date)):
            date = None
        return {
            "_id": celex,
            "_source": "EU/EP-Decisions",
            "_type": "doctrine",
...
def main():
    ...
    if command in ("bootstrap", "bootstrap-fast"):
        ...
        print(f"SPARQL returned {len(page)} EP decisions on page 1")
        if page:
            c = page[0]["celex"]
