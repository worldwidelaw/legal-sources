#!/usr/bin/env python3
"""
EU/ComplementaryLegislation — the EU's "complementary legislation" (CELEX
**sector 4**).

Beyond the Treaties (sector 1), the external agreements (sector 2) and the
enacted secondary legislation of the institutions (sector 3, EU/EUR-Lex), the
EUR-Lex taxonomy carries a distinct fourth sector — **complementary
legislation** — for binding acts that are not secondary legislation of a single
institution. It is dominated by three well-defined classes, all published in the
Official Journal and registered in the EU Publications Office repository
(CELLAR) with a CELEX number of the form ``4{YYYY}{DESC}{NNNN}``:

* **``X`` — UN/UNECE Regulations** given legal effect in the EU through its
  accession to the UNECE 1958 Agreement (uniform technical prescriptions for the
  type-approval of motor vehicles, their equipment and parts — braking systems,
  lighting, safety-glazing, emissions, etc.). These are substantial, directly
  applicable regulatory texts.
* **``D`` — Decisions of the Representatives of the Governments of the Member
  States** meeting within the Council (e.g. appointing Judges and
  Advocates-General to the Court of Justice and the General Court, or seats of
  EU bodies).
* **``Y`` — Resolutions of the Council and of the Representatives of the
  Governments of the Member States** meeting within the Council on matters of
  common policy.

This is a distinct, additive corpus: EU/EUR-Lex covers only enacted REG/DIR/DEC
(sector 3), EU/InternationalAgreements the sector-2 ``A`` agreements and
EU/JointBodyDecisions the sector-2 ``D`` decisions; sector-4 complementary
legislation is not otherwise pulled.

Data flow
---------
1. Enumerate every sector-4 act via the public CELLAR SPARQL endpoint (CELEX
   matching ``^4[0-9]{4}``), retrieving CELEX + document date + English
   expression title. Paged with LIMIT/OFFSET (the sector-4 corpus is ~1,650
   rows, well under the ~10 000-row SPARQL OFFSET ceiling, so no year-scoping is
   needed).
...
Same CELLAR recipe as EU/InternationalAgreements, EU/JointBodyDecisions and
EU/EP-Decisions, narrowed to the sector-4 CELEX descriptor. Corrigenda
(``…R(01)``) and metadata-only stubs have no xHTML manifestation and 404 →
skipped by the minimum-length guard.
"""
...
# CELEX pattern for sector-4 complementary legislation: 4{YYYY}{DESC}{...}
CELEX_RE = r"^4[0-9]{4}"
...
class ComplementaryLegislationScraper(BaseScraper):
    """Scraper for EU complementary legislation (CELEX sector 4) via CELLAR."""
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield RAW act dicts (with full text) for every sector-4 act."""
        offset = 0
        seen = set()
        while True:
    ...
    def normalize(self, raw: dict) -> dict:
        ...
        if not title:
            title = f"Complementary-legislation act {celex}"
        date = raw.get("date")
        if date and not re.match(r"^\d{4}-\d{2}-\d{2}", str(date)):
            date = None
        return {
            "_id": celex,
            "_source": "EU/ComplementaryLegislation",
            "_type": "legislation",
...
def main():
    ...
    scraper = ComplementaryLegislationScraper()
    ...
    if command in ("bootstrap", "bootstrap-fast"):
        ...
        print(f"SPARQL returned {len(page)} sector-4 acts on page 1")
        if page:
            c = page[0]["celex"]
