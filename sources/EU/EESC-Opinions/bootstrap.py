...
The EESC adopts ~160-190 advisory opinions, information reports and
resolutions a year (Council/Commission/Parliament referrals + own-initiative).
...
Data flow
---------
1. Enumerate every EESC opinion via the public CELLAR SPARQL endpoint
   (CELEX matching ``^5[0-9]{4}AE``), retrieving CELEX + document date +
...
Why not the EESC DM Search SPA? The public listing at eesc.europa.eu is an
Angular "DM Search v5.5.1" app whose enumeration XHR is Azure-AD gated
(dm-api.eesc.europa.eu). CELLAR is the authoritative, anonymous, datacenter-
friendly full-text source for the same corpus.
"""
...
# CELEX pattern for EESC opinions: 5{YYYY}AE{NNNN}
CELEX_RE = r"^5[0-9]{4}AE"
...
class EESCOpinionsScraper(BaseScraper):
    """Scraper for EESC opinions via CELLAR (Publications Office)."""
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield RAW opinion dicts (with full text) for every EESC opinion."""
        offset = 0
        seen = set()
        while True:
