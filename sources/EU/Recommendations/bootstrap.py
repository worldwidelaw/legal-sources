#!/usr/bin/env python3
"""
EU/Recommendations — Recommendations of the EU institutions (CELEX **sector 3**,
document class ``H``).

Under Article 288 TFEU the Union's acts include **recommendations**, which "have
no binding force". They are nonetheless authoritative soft-law instruments issued
by the Council, the Commission and the ECB — e.g. the Commission Recommendation
...
conduct, etc. They are published in the Official Journal and registered in the EU
Publications Office repository (CELLAR) with a CELEX number of the form
``3{YYYY}H{NNNN}`` (sector 3 = secondary legislation of the institutions; ``H`` =
the recommendation document class).
...
variants) and therefore never pulls recommendations; the modern advisory-body
opinions (EESC/CoR/ECB) live under their own descriptors. Sector-3 ``H``
recommendations — ~1,770 acts spanning 1960 to the present — are not otherwise
captured.

Data flow
---------
1. Enumerate every recommendation via the public CELLAR SPARQL endpoint (CELEX
   matching ``^3[0-9]{4}H[0-9]``), retrieving CELEX + document date + English
   expression title. The corpus is ~1,770 rows, well under the ~10 000-row SPARQL
   OFFSET ceiling, so a single paged LIMIT/OFFSET sweep suffices (no year-scoping).
2. Fetch the full text of each act from CELLAR via HTTP content negotiation.
   Modern recommendations are served as OJ/Formex **xHTML**; older ones (pre-2005)
   as an **OJ HTML** manifestation addressed by the language-suffixed CELEX
...
Same CELLAR recipe as EU/ESC-Opinions and EU/ComplementaryLegislation, narrowed
to the sector-3 ``H`` recommendation descriptor. Corrigenda (``…R(01)``) and
...
# CELEX pattern for sector-3 recommendations: 3{YYYY}H{NNNN}[(NN)].
CELEX_RE = r"^3[0-9]{4}H[0-9]"
...
class RecommendationsScraper(BaseScraper):
    """Scraper for EU recommendations (CELEX sector-3 ``H``) via CELLAR."""
    ...
    def _fetch_text(self, celex: str) -> str:
        ...
        Modern recommendations carry an xHTML (OJ/Formex) manifestation; older
        ...
        # 1) xHTML (OJ/Formex bodies) — the usual case for modern recommendations.
        r = self._get(
        ...
        # 2) OJ HTML via language-suffixed CELEX (older recommendations).
        r = self._get(
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield RAW recommendation dicts (with full text), newest-first."""
        offset = 0
        seen = set()
        while True:
    ...
    def fetch_updates(self, since: datetime) -> Generator[dict, None, None]:
        """Yield recommendations with a document date on/after ``since`` (newest-first)."""
    ...
    def normalize(self, raw: dict) -> dict:
        ...
        if not title:
            title = f"EU Recommendation {celex}"
        date = raw.get("date")
        if date and not re.match(r"^\d{4}-\d{2}-\d{2}", str(date)):
            date = None
        return {
            "_id": celex,
            "_source": "EU/Recommendations",
            "_type": "doctrine",
...
def main():
    ...
    scraper = RecommendationsScraper()
    ...
    if command in ("bootstrap", "bootstrap-fast"):
        ...
        print(f"SPARQL returned {len(page)} recommendations on page 1")
        if page:
            c = page[0]["celex"]
