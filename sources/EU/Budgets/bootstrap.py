#!/usr/bin/env python3
"""
EU/Budgets — EU budget statements, amending budgets and definitive adoptions
(CELEX **sector 3**, document class ``B``).

The sector-3 ``B`` descriptor catalogues the Union's **budgetary** acts published
in the Official Journal: the *statements of revenue and expenditure* of the EU
...
of the European Union. Examples: the statement of revenue and expenditure for the
Fusion for Energy Joint Undertaking (F4E), for the EU Agency for Fundamental
Rights (FRA), for Europol, and the definitive adoption of the Union's annual
general budget.
...
variants) and therefore never pulls budgetary acts. C-series acts under the
``Y`` descriptor (EU/OJC-Acts) and Council resolutions under ``G``
(EU/CouncilResolutions) are *different* sets of CELEX works — no ``_id``
collision — so ``B`` is not otherwise captured.
...
2. Fetch the full text of each act from CELLAR via HTTP content negotiation:
   ...
   fleet-safe.
3. Normalize to the standard schema (doctrine — official budgetary documents
   published in the OJ; not binding secondary legislation).

Same CELLAR recipe as EU/CouncilResolutions and EU/OJC-Acts, narrowed to the
sector-3 ``B`` budgetary descriptor. Corrigenda (``…R(01)``) and metadata-only
...
# CELEX pattern for sector-3 budgetary acts: 3{YYYY}B{NNNN}[(NN)].
CELEX_RE = r"^3[0-9]{4}B[0-9]"
...
class BudgetsScraper(BaseScraper):
    """Scraper for EU budgetary acts (CELEX sector-3 ``B``) via CELLAR."""
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield RAW budgetary-act dicts (with full text), newest-first."""
        offset = 0
        seen = set()
        while True:
    ...
    def normalize(self, raw: dict) -> dict:
        ...
        if not title:
            title = f"EU budgetary act {celex}"
        date = raw.get("date")
...
def main():
    ...
    if command in ("bootstrap", "bootstrap-fast"):
        ...
        print(f"SPARQL returned {len(page)} EU budgetary acts on page 1")
        if page:
            c = page[0]["celex"]
