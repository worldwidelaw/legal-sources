#!/usr/bin/env python3
"""
EU/SWD-Documents — European Commission Staff Working Documents (SWD/SEC).

The Commission publishes several hundred *staff working documents* a year:
impact assessments, evaluations, fitness checks, executive summaries, and the
older "SEC" series. Each is registered in the EU Publications Office repository
(CELLAR) with a CELEX number of the form ``5{YYYY}SC{NNNN}`` (sector 5 =
preparatory acts; "SC" = the SWD/SEC document class). This is the same corpus
that fills eur-lex.europa.eu's "Preparatory documents → Staff working
documents" facet.

Data flow
---------
1. Enumerate every SWD/SEC document via the public CELLAR SPARQL endpoint
   (CELEX matching ``^5{YEAR}SC``), one publication year at a time to stay well
...
2. Fetch the full text from CELLAR via HTTP content negotiation:
     * first ``Accept: application/xhtml+xml`` (recent SWDs served as OJ/Formex
       xHTML bodies), else
...
Why not the eur-lex SWD facet directly? Its listing/download pages sit behind
the AWS-WAF; CELLAR is the authoritative, anonymous, datacenter-friendly
full-text source for the same documents. This mirrors the EU/COM-Documents
recipe (same PDF fallback), narrowed to the "SC" descriptor.
"""
...
# SEC/SWD documents exist from the 1970s onward; go a little earlier to be safe.
MIN_YEAR = 1975
...
# Leading token in the Formex/xHTML export is the source filename, e.g.
# "IMMC.SWD%282023%29207%20final.ENG.xhtml.1_EN_resume..._part1_v3.docx" — strip it.
...
class SWDDocumentsScraper(BaseScraper):
    """Scraper for European Commission Staff Working Documents via CELLAR."""
    ...
    def _enumerate_year(self, year: int) -> Generator[dict, None, None]:
        """Yield SWD/SEC document metadata for a single year."""
        pattern = f"^5{year}SC"
        offset = 0
        seen = set()
        while True:
    ...
    def _fetch_text(self, celex: str) -> str:
        ...
        # 1) xHTML (recent SWDs served as OJ/Formex bodies).
        r = self._get(
        ...
        # 2) PDF streams (most SWDs have no xHTML manifestation).
        if fitz is None:
            return ""
        r = self._get(
            CELLAR_CELEX.format(celex=celex),
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield RAW SWD-document dicts (with full text), newest year first."""
    ...
    def fetch_updates(self, since: datetime) -> Generator[dict, None, None]:
        """Yield SWD documents from the years spanning ``since`` to now.

        SWD CELEX years map directly to publication years, so we only rescan the
        affected years; the loader dedups on _id.
        """
    ...
    def normalize(self, raw: dict) -> dict:
        ...
        if not title:
            title = f"Commission Staff Working Document {celex}"
        date = raw.get("date")
        if date and not re.match(r"^\d{4}-\d{2}-\d{2}", str(date)):
            date = None
        return {
            "_id": celex,
            "_source": "EU/SWD-Documents",
            "_type": "doctrine",
...
def main():
    ...
    if command in ("bootstrap", "bootstrap-fast"):
        ...
        page = scraper._sparql_page(f"^5{scraper._current_year()}SC", 0)
        print(f"SPARQL returned {len(page)} SWDs on page 1 for the current year")
        if page:
            c = page[0]["celex"]
