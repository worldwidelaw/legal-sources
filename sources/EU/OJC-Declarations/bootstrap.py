#!/usr/bin/env python3
"""
EU/OJC-Declarations — declarations and joint statements entered in the Official Journal
**C series**, catalogued under CELEX **sector 3**, document class ``C``.

The ``C`` descriptor gathers the **declarations, joint statements and statements**
made by the EU institutions — chiefly the **Council** and the **Commission**, alone or
jointly, and the interinstitutional joint statements of the European Parliament, the
Council and the Commission — that are entered in the "Information and Notices" (**C**)
series of the Official Journal (CELEX ``3{YYYY}C{NNNN}``, e.g. ``32013C1220(02)`` =
"Declarations of the Commission" annexed to a legislative act, or ``32026C03482`` = a
"Joint Statement by the European Parliament, the Council and the Commission"). Typical
content: declarations entered in Council minutes on the interpretation/application of an
adopted act, Commission statements on the implementation of a regulation or directive,
joint statements on the annual budgetary procedure, and statements on the exercise of
delegated/implementing powers. These are soft-law/interpretative acts, not the binding
enacting text — hence classified as **doctrine**.

This is a distinct, additive corpus. EU/EUR-Lex enumerates sector 3 by the ordinary
binding resource-types only (REG / DIR / DEC and their implementing/delegated variants)
and therefore never pulls the ``C`` declarations. ``C`` is a different CELEX letter from
every other descriptor (S, E, F, H, Q, Y, …), so there is no ``_id`` collision with the
sibling sector-3 sources (it is in particular distinct from EU/OJC-Acts, which is the
``Y`` descriptor — OJ C-series *acts* such as resolutions and conclusions).

Data flow
---------
1. Enumerate every act via the public CELLAR SPARQL endpoint (CELEX matching
   ``^3[0-9]{4}C[0-9]``). The corpus is ~170 rows, far under the SPARQL OFFSET
...
2. Fetch the full text of each act from CELLAR via HTTP content negotiation:
   ...
   fleet-safe.
3. Normalize to the standard schema (doctrine — declarations / joint statements).

Same CELLAR recipe as EU/ECSC-Decisions, EU/CFSP-Acts and EU/RulesOfProcedure, narrowed
to the sector-3 ``C`` descriptor. Parenthesised CELEX sub-parts (``…(02)``) and older
acts are handled; corrigenda (``…R(01)``) and metadata-only stubs resolve to no usable
manifestation and are skipped by the minimum-length guard.
"""
...
# CELEX pattern for sector-3 OJ C-series declarations: 3{YYYY}C{NNNN}[(NN)].
CELEX_RE = r"^3[0-9]{4}C[0-9]"
...
#   "EUR-Lex - 32000C0630 - EN | Avis juridique important | 32000C0630 ..."
...
class OJCDeclarationsScraper(BaseScraper):
    """Scraper for EU OJ C-series declarations (CELEX sector-3 ``C``) via CELLAR."""
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield RAW declaration dicts (with full text), newest-first."""
        offset = 0
        seen = set()
        while True:
    ...
    def normalize(self, raw: dict) -> dict:
        ...
        if not title:
            title = f"OJ C-series declaration {celex}"
        date = raw.get("date")
        if date and not re.match(r"^\d{4}-\d{2}-\d{2}", str(date)):
            date = None
        return {
            "_id": celex,
            "_source": "EU/OJC-Declarations",
            "_type": "doctrine",
...
def main():
    ...
    scraper = OJCDeclarationsScraper()
    ...
    if command in ("bootstrap", "bootstrap-fast"):
        ...
        print(f"SPARQL returned {len(page)} OJ C-series declarations on page 1")
        if page:
            c = page[0]["celex"]
