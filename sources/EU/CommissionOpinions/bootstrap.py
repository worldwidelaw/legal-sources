#!/usr/bin/env python3
"""
EU/CommissionOpinions — Opinions of the European Commission (CELEX **sector 3**,
document class ``A``).

Under Article 288 TFEU the Union's acts include **opinions**, which "have no
binding force". The European Commission issues formal opinions on a wide range of
Treaty and secondary-law bases and publishes them in the Official Journal —
e.g. Commission opinions on the **draft budgetary plans** of euro-area Member
States (Regulation (EU) No 473/2013), opinions relating to plans for the
**disposal of radioactive waste** under Article 37 of the Euratom Treaty,
opinions on requests to **amend the statutes of national central banks**,
opinions on **draft amendments to the Treaties** under Article 48 TEU, and
opinions on accession applications. They are registered in the EU Publications
Office repository (CELLAR) with a CELEX number of the form ``3{YYYY}A{NNNN}``
(sector 3 = secondary legislation of the institutions; ``A`` = the opinion
document class).

This is a distinct, additive corpus. EU/EUR-Lex enumerates sector 3 by the
binding resource-types only (REG / DIR / DEC and their implementing/delegated
variants) and therefore never pulls Commission opinions; the advisory-body
opinions (EESC/CoR/ECB) live under their own sector-5 descriptors (AE / AR / AB).
Sector-3 ``A`` Commission opinions — ~655 acts — are not otherwise captured.

Data flow
---------
1. Enumerate every opinion via the public CELLAR SPARQL endpoint (CELEX matching
   ``^3[0-9]{4}A[0-9]``), retrieving CELEX + document date + English expression
   title. The corpus is ~655 rows, well under the ~10 000-row SPARQL OFFSET
   ceiling, so a single paged LIMIT/OFFSET sweep suffices (no year-scoping).
2. Fetch the full text of each act from CELLAR via HTTP content negotiation.
   Modern opinions are served as OJ/Formex **xHTML**; older ones as an **OJ HTML**
   manifestation addressed by the language-suffixed CELEX
   (``/resource/celex/{CELEX}.ENG``), and a minority only as a born-digital
   **PDF** stream (PyMuPDF). Each is tried in turn. All are served by CELLAR,
   which bypasses the eur-lex.europa.eu AWS-WAF that 202-challenges datacenter
   IPs, so it is fleet-safe.
3. Normalize to the standard schema (doctrine — non-binding soft law).

Same CELLAR recipe as EU/Recommendations and EU/ESC-Opinions, narrowed to the
sector-3 ``A`` Commission-opinion descriptor. Corrigenda (``…R(01)``) and
metadata-only stubs resolve to no usable manifestation and are skipped by the
...
# CELEX pattern for sector-3 Commission opinions: 3{YYYY}A{NNNN}[(NN)].
CELEX_RE = r"^3[0-9]{4}A[0-9]"
...
#   "EUR-Lex - 32003H0361 - EN | Avis juridique important | 32003H0361 ..."
...
class CommissionOpinionsScraper(BaseScraper):
    """Scraper for EU Commission opinions (CELEX sector-3 ``A``) via CELLAR."""
    ...
    def _fetch_text(self, celex: str) -> str:
        ...
        Modern opinions carry an xHTML (OJ/Formex) manifestation; older
        ones an OJ HTML manifestation addressed by the language-suffixed CELEX;
        a minority only a born-digital PDF. Try each in turn.
        """
        ...
        # 2) OJ HTML via language-suffixed CELEX (older opinions).
        r = self._get(
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield RAW Commission-opinion dicts (with full text), newest-first."""
        offset = 0
        seen = set()
        while True:
    ...
    def normalize(self, raw: dict) -> dict:
        ...
        if not title:
            title = f"Commission Opinion {celex}"
        date = raw.get("date")
        if date and not re.match(r"^\d{4}-\d{2}-\d{2}", str(date)):
            date = None
        return {
            "_id": celex,
            "_source": "EU/CommissionOpinions",
            "_type": "doctrine",
...
def main():
    ...
    scraper = CommissionOpinionsScraper()
    ...
    if command in ("bootstrap", "bootstrap-fast"):
        ...
        print(f"SPARQL returned {len(page)} Commission opinions on page 1")
        if page:
            c = page[0]["celex"]
