...
Within the ordinary legislative procedure the Council of the European Union
adopts a *position at first reading* (historically a *common position*) on a
proposed regulation, directive or decision, together with a *statement of the
Council's reasons*. These are formal preparatory acts published in the OJ C
series and registered in the EU Publications Office repository (CELLAR) with a
CELEX number of the form ``5{YYYY}AG{NNNN}`` (sector 5 = preparatory acts;
"AG" = the Council-position document class). This is the same corpus that fills
eur-lex.europa.eu's "Preparatory documents → Council positions" facet.

Data flow
---------
1. Enumerate every Council position via the public CELLAR SPARQL endpoint
   (CELEX matching ``^5{YEAR}AG``), one publication year at a time to stay well
...
source for the same documents. This mirrors the EU/COM-Documents and
EU/SWD-Documents recipe, narrowed to the "AG" descriptor.
"""
...
# Council common positions appear from the cooperation procedure (SEA 1987)
# onward; go a little earlier to be safe.
MIN_YEAR = 1987
...
class CouncilPositionsScraper(BaseScraper):
    """Scraper for Council of the EU positions at first reading via CELLAR."""
    ...
    def _enumerate_year(self, year: int) -> Generator[dict, None, None]:
        """Yield Council-position metadata for a single year."""
        pattern = f"^5{year}AG"
        offset = 0
        seen = set()
        while True:
    ...
    def _fetch_text(self, celex: str) -> str:
        ...
        Council-position CELEX numbers carry a parenthesised sub-part suffix
        (e.g. ``52023AG0002(01)``); the ``/resource/celex/`` endpoint only
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield RAW Council-position dicts (with full text), newest year first."""
    ...
    def fetch_updates(self, since: datetime) -> Generator[dict, None, None]:
        """Yield Council positions from the years spanning ``since`` to now.

        Council-position CELEX years map directly to publication years, so we
        only rescan the affected years; the loader dedups on _id.
        """
    ...
    def normalize(self, raw: dict) -> dict:
        ...
        if not title:
            title = f"Position of the Council {celex}"
        date = raw.get("date")
        if date and not re.match(r"^\d{4}-\d{2}-\d{2}", str(date)):
            date = None
        return {
            "_id": celex,
            "_source": "EU/Council-Positions",
            "_type": "doctrine",
...
def main():
    ...
    scraper = CouncilPositionsScraper()
    ...
    if command in ("bootstrap", "bootstrap-fast"):
        ...
        page = scraper._sparql_page(f"^5{scraper._current_year()}AG", 0)
        print(f"SPARQL returned {len(page)} Council positions on page 1 for the current year")
        if not page:
            # current year may have none yet; probe a known-populated year
            page = scraper._sparql_page("^52015AG", 0)
            print(f"  fallback ^52015AG page 1: {len(page)} rows")
        if page:
            c = page[0]["celex"]
