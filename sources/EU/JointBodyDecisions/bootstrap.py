#!/usr/bin/env python3
"""
EU/JointBodyDecisions — decisions of bodies established by EU international
agreements, and Council decisions in external relations (CELEX **sector 2**,
``D`` descriptor).

Alongside the agreement texts themselves (EU/InternationalAgreements, sector-2
``A``), the external-relations sector of EUR-Lex holds the **decisions** taken to
administer and extend those agreements. The corpus is dominated by decisions of
the joint bodies set up by EU agreements — above all the **EEA Joint Committee**
(which incorporates EU legislation into the EEA Agreement, extending it to
Norway, Iceland and Liechtenstein), together with **Association Council** and
**Cooperation Council** decisions under the EU's association/partnership
agreements, plus Council decisions in external relations. Their authentic text is
published in the OJ L series with a CELEX number in **sector 2**, document class
``D`` — ``2{YYYY}D{NNNN}`` (e.g. ``22026D1299`` — EEA Joint Committee Decision
No 107/2026).

This is a distinct, additive corpus: EU/EUR-Lex covers sector-3 legislation and
EU/InternationalAgreements the sector-2 ``A`` agreements; these joint-body/Council
decisions live in sector-2 ``D`` and are not otherwise pulled.

Data flow
---------
1. Enumerate every sector-2 ``D`` act via the public CELLAR SPARQL endpoint
   (CELEX matching ``^2[0-9]{4}D``), retrieving CELEX + document date + English
   expression title. Paged with LIMIT/OFFSET (the ``D`` corpus is ~8,900 rows,
   under the ~10 000-row SPARQL OFFSET ceiling, so no year-scoping is needed).
...
Same CELLAR recipe as EU/InternationalAgreements, EU/EESC-Opinions and
EU/EP-Decisions, narrowed to the sector-2 ``D`` descriptor. Corrigenda
...
# CELEX pattern for sector-2 D acts: 2{YYYY}D{...}
CELEX_RE = r"^2[0-9]{4}D"
...
class JointBodyDecisionsScraper(BaseScraper):
    """Scraper for EU external-relations decisions (sector-2 D) via CELLAR."""
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield RAW decision dicts (with full text) for every sector-2 D act."""
        offset = 0
        seen = set()
        while True:
    ...
    def normalize(self, raw: dict) -> dict:
        ...
        if not title:
            title = f"External-relations decision {celex}"
        date = raw.get("date")
        if date and not re.match(r"^\d{4}-\d{2}-\d{2}", str(date)):
            date = None
        return {
            "_id": celex,
            "_source": "EU/JointBodyDecisions",
            "_type": "legislation",
...
def main():
    ...
    scraper = JointBodyDecisionsScraper()
    ...
    if command in ("bootstrap", "bootstrap-fast"):
        ...
        print(f"SPARQL returned {len(page)} sector-2 D acts on page 1")
        if page:
            c = page[0]["celex"]
