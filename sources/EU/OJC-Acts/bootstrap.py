#!/usr/bin/env python3
"""
EU/OJC-Acts — Acts published in the Official Journal **C series** (CELEX
**sector 3**, document class ``Y``).

The Official Journal of the European Union is issued in two series: the **L
series** ("Legislation" — binding regulations, directives and decisions) and the
**C series** ("Information and Notices"). CELLAR/EUR-Lex catalogue the acts of
the institutions that appear in the **C series** under the sector-3 document
descriptor ``Y`` (CELEX ``3{YYYY}Y{NNNN}``). This is a large, heterogeneous body
of authoritative non-binding acts and official government-authored texts, e.g.:
...
* **Commission communications and notices** published in the C series (state-aid
  notices, air-service public-service-obligation notices, anti-dumping notices,
  interpretative and explanatory communications);
...
* the **rules of procedure** of Union bodies.
...
variants) and therefore never pulls C-series ``Y`` acts. The small ESRB subset
(~5 %) partially overlaps EU/ESRB, but that source keys on its own portal IDs so
the loader keeps both; everything else here — Council resolutions/conclusions,
Commission C-series communications, Court of Auditors special reports,
interinstitutional agreements — is not otherwise captured.

Data flow
---------
1. Enumerate every C-series act via the public CELLAR SPARQL endpoint (CELEX
   matching ``^3[0-9]{4}Y[0-9]``), retrieving CELEX + document date + English
   expression title. The corpus is ~2,088 rows, well under the ~10 000-row
   SPARQL OFFSET ceiling, so a single paged LIMIT/OFFSET sweep suffices (no
   year-scoping).
2. Fetch the full text of each act from CELLAR via HTTP content negotiation.
   Modern acts are served as OJ/Formex **xHTML**; older ones as an **OJ HTML**
...
Same CELLAR recipe as EU/CommissionOpinions and EU/Recommendations, narrowed to
the sector-3 ``Y`` C-series descriptor. Corrigenda (``…R(01)``) and metadata-only
stubs resolve to no usable manifestation and are skipped by the minimum-length
guard.
"""
...
# CELEX pattern for sector-3 OJ C-series acts: 3{YYYY}Y{NNNN}[(NN)].
CELEX_RE = r"^3[0-9]{4}Y[0-9]"
...
class OJCActsScraper(BaseScraper):
    """Scraper for EU OJ C-series acts (CELEX sector-3 ``Y``) via CELLAR."""
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield RAW C-series-act dicts (with full text), newest-first."""
        offset = 0
        seen = set()
        while True:
...
def main():
    ...
    if command in ("bootstrap", "bootstrap-fast"):
        ...
        print(f"SPARQL returned {len(page)} OJ C-series acts on page 1")
        if page:
            c = page[0]["celex"]
