#!/usr/bin/env python3
"""
EU/CFSP-Acts — Common Foreign and Security Policy joint actions and common positions
(CELEX **sector 3**, document class ``E``).

The ``E`` descriptor gathers the EU's **second-pillar** Common Foreign and Security
Policy (CFSP, Title V TEU) acts: the Council **joint actions** and **common positions**
(and the earlier CFSP decisions carrying this class) adopted from the mid-1990s until
the Lisbon Treaty reorganised CFSP instruments. They cover **restrictive measures /
sanctions** against third countries and persons, the mandates of **ESDP/CSDP crisis-
management missions and operations** (e.g. EUFOR, EUPOL, EU Special Representatives),
non-proliferation and arms-control actions, and amendments/extensions/repeals of those
instruments. They are published in the Official Journal and catalogued by the EU
Publications Office (CELLAR / EUR-Lex) under the sector-3 descriptor ``E`` (CELEX
``3{YYYY}E{NNNN}``, e.g. ``32009E0788`` = Common Position 2009/788/CFSP).
...
and therefore never pulls the ``E`` CFSP joint actions and common positions — they carry
the dedicated JOINT_ACTION / COMMON_POSITION resource types, not DEC. Distinct too from
EU/FrameworkDecisions (descriptor ``F`` = the third-pillar / JHA framework decisions):
``E`` and ``F`` are different CELEX letters, so no ``_id`` collision with any descriptor.
...
2. Fetch the full text of each act from CELLAR via HTTP content negotiation:
   ...
   fleet-safe.
3. Normalize to the standard schema (legislation — binding CFSP acts).

Same CELLAR recipe as EU/FrameworkDecisions and EU/RulesOfProcedure, narrowed to
the sector-3 ``E`` descriptor. Corrigenda (``…R(01)``) and metadata-only stubs
...
# CELEX pattern for sector-3 CFSP joint actions / common positions: 3{YYYY}E{NNNN}[(NN)].
CELEX_RE = r"^3[0-9]{4}E[0-9]"
...
class CFSPActsScraper(BaseScraper):
    """Scraper for EU CFSP joint actions / common positions (CELEX sector-3 ``E``) via CELLAR."""
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield RAW CFSP-act dicts (with full text), newest-first."""
        offset = 0
        seen = set()
        while True:
    ...
    def normalize(self, raw: dict) -> dict:
        ...
        if not title:
            title = f"CFSP Joint Action / Common Position {celex}"
        date = raw.get("date")
        if date and not re.match(r"^\d{4}-\d{2}-\d{2}", str(date)):
            date = None
        return {
            "_id": celex,
            "_source": "EU/CFSP-Acts",
            "_type": "legislation",
...
def main():
    ...
    if command in ("bootstrap", "bootstrap-fast"):
        ...
        print(f"SPARQL returned {len(page)} CFSP joint actions / common positions on page 1")
        if page:
            c = page[0]["celex"]
