...
Within the ordinary (and historic cooperation / codecision) legislative
procedure the European Parliament adopts a *legislative resolution* — its
*position at first or second reading* — on a proposed regulation, directive or
decision. These are formal preparatory acts published in the OJ C series and
registered in the EU Publications Office repository (CELLAR) with a CELEX number
of the form ``5{YYYY}AP{NNNN}`` (sector 5 = preparatory acts; "AP" = the
European-Parliament-position document class). They carry the ``P{n}_TA(YYYY)NNNN``
"texts adopted" identifiers and are the Parliament-side counterpart to the
Commission proposals in EU/COM-Documents and the Council positions in
EU/Council-Positions. This is the same corpus that fills eur-lex.europa.eu's
"Preparatory documents → European Parliament positions" facet.

Data flow
---------
1. Enumerate every EP position via the public CELLAR SPARQL endpoint
   (CELEX matching ``^5{YEAR}AP``), one publication year at a time to stay well
...
2. Fetch the full text from CELLAR via HTTP content negotiation:
     * first ``Accept: application/xhtml+xml`` (positions served as OJ/Formex
       xHTML bodies), else
     * ``Accept: application/pdf`` — a direct born-digital PDF, or a
       ``300 Multiple-Choice`` listing from which we pick the English "part1"
       PDF stream and extract with PyMuPDF.
...
EU/SWD-Documents and EU/Council-Positions recipe, narrowed to the "AP"
descriptor.

Distinct from EU/EuroParl (EP Open Data API adopted-texts, 2014+ only): AP
provides the canonical OJ-published legislative resolutions, CELEX-registered,
and extends back to the 1990s — thousands of pre-2014 documents EuroParl lacks.
"""
...
# EP legislative resolutions in the OJ C series appear from the cooperation
# procedure (SEA 1987) onward; go a little earlier to be safe.
MIN_YEAR = 1987
...
class EPPositionsScraper(BaseScraper):
    """Scraper for European Parliament positions / legislative resolutions via CELLAR."""
    ...
    def _enumerate_year(self, year: int) -> Generator[dict, None, None]:
        """Yield EP-position metadata for a single year."""
        pattern = f"^5{year}AP"
        offset = 0
        seen = set()
        while True:
    ...
    def _fetch_text(self, celex: str) -> str:
        ...
        Some EP-position CELEX numbers carry a parenthesised sub-part suffix
        (e.g. ``52017AP0001(01)``); the ``/resource/celex/`` endpoint only
        resolves them when the CELEX is URL-encoded.
        """
        enc = quote(celex, safe="")

        # 1) xHTML (OJ/Formex bodies).
        r = self._get(
        ...
        # 2) PDF streams (older positions or missing xHTML manifestation).
        if fitz is None:
            return ""
        r = self._get(
            CELLAR_CELEX.format(celex=enc),
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield RAW EP-position dicts (with full text), newest year first."""
    ...
    def fetch_updates(self, since: datetime) -> Generator[dict, None, None]:
        """Yield EP positions from the years spanning ``since`` to now.

        EP-position CELEX years map directly to publication years, so we only
    ...
    def normalize(self, raw: dict) -> dict:
        ...
        if not title:
            title = f"Position of the European Parliament {celex}"
        date = raw.get("date")
        if date and not re.match(r"^\d{4}-\d{2}-\d{2}", str(date)):
            date = None
        return {
            "_id": celex,
            "_source": "EU/EP-Positions",
            "_type": "doctrine",
...
def main():
    ...
    if command in ("bootstrap", "bootstrap-fast"):
        ...
        page = scraper._sparql_page(f"^5{scraper._current_year()}AP", 0)
        print(f"SPARQL returned {len(page)} EP positions on page 1 for the current year")
        if not page:
            # current year may have none yet; probe a known-populated year
            page = scraper._sparql_page("^52011AP", 0)
            print(f"  fallback ^52011AP page 1: {len(page)} rows")
        if page:
            c = page[0]["celex"]
