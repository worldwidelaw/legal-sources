#!/usr/bin/env python3
"""
EU/ECSC-Decisions — general (normative) decisions under the European Coal and Steel
Community Treaty (CELEX **sector 3**, document class ``S``).

The ``S`` descriptor gathers the **general decisions** adopted under the ECSC Treaty
(Treaty of Paris, 1952-2002): first by the ECSC **High Authority** and, after the
1967 Merger Treaty, by the **Commission**, acting as ECSC-Treaty law-maker. Unlike
ordinary EEC/EC decisions (descriptor ``D``, addressed to specific parties), the ECSC
"general decisions" were the ECSC's equivalent of regulations — normative acts of
general application (numbered ``No NNNN/YY/ECSC``) governing the common market in coal
and steel: levies on production, State-aid rules for the coal/steel industries,
production quotas and price rules, external-trade / anti-dumping measures on steel
imports, and their amendments/extensions. They were published in the Official Journal
and catalogued by the EU Publications Office (CELLAR / EUR-Lex) under the sector-3
descriptor ``S`` (CELEX ``3{YYYY}S{NNNN}``, e.g. ``32002S1469`` = Commission Decision
No 1469/2002/ECSC). The corpus runs from the 1950s until the ECSC Treaty expired on
23 July 2002.
...
and therefore never pulls the ``S`` ECSC general decisions — they carry the dedicated
ECSC-decision resource type, not DEC. ``S`` is a different CELEX letter from every other
descriptor (D, E, F, H, …), so there is no ``_id`` collision.

Data flow
---------
1. Enumerate every act via the public CELLAR SPARQL endpoint (CELEX matching
   ``^3[0-9]{4}S[0-9]``). The corpus is ~590 rows, far under the SPARQL OFFSET
...
2. Fetch the full text of each act from CELLAR via HTTP content negotiation:
   ...
   fleet-safe.
3. Normalize to the standard schema (legislation — binding ECSC general decisions).

Same CELLAR recipe as EU/CFSP-Acts, EU/FrameworkDecisions and EU/RulesOfProcedure,
narrowed to the sector-3 ``S`` descriptor. Corrigenda (``…R(01)``) and metadata-only
stubs resolve to no usable manifestation and are skipped by the minimum-length guard.
"""
...
# CELEX pattern for sector-3 ECSC general decisions: 3{YYYY}S{NNNN}[(NN)].
CELEX_RE = r"^3[0-9]{4}S[0-9]"
...
class ECSCDecisionsScraper(BaseScraper):
    """Scraper for EU ECSC general decisions (CELEX sector-3 ``S``) via CELLAR."""
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield RAW ECSC-decision dicts (with full text), newest-first."""
        offset = 0
        seen = set()
        while True:
    ...
    def fetch_updates(self, since: datetime) -> Generator[dict, None, None]:
        ...
        The ECSC Treaty expired on 23 July 2002, so this corpus is effectively
        historical/closed; ``fetch_updates`` remains implemented for contract
        completeness and any late catalogue additions.
        """
    ...
    def normalize(self, raw: dict) -> dict:
        ...
        if not title:
            title = f"ECSC Decision {celex}"
        date = raw.get("date")
        if date and not re.match(r"^\d{4}-\d{2}-\d{2}", str(date)):
            date = None
        return {
            "_id": celex,
            "_source": "EU/ECSC-Decisions",
            "_type": "legislation",
...
def main():
    ...
    if command in ("bootstrap", "bootstrap-fast"):
        ...
        print(f"SPARQL returned {len(page)} ECSC general decisions on page 1")
        if page:
            c = page[0]["celex"]
