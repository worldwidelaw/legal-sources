#!/usr/bin/env python3
"""
EU/EESC-InitiativeOpinions — European Economic and Social Committee own-initiative
and exploratory opinions (sector-5 CELEX ``IE`` class).

The EESC issues two broad classes of opinions. Its *referral* opinions — adopted
in response to a mandatory or optional consultation by the Council, Commission or
Parliament on a specific legislative file — carry the CELEX descriptor ``AE``
(*avis CESE*) and are covered by EU/EESC-Opinions. Distinct from those, the
Committee adopts **own-initiative and exploratory opinions**, in which it takes
up a subject on its own motion (or at the exploratory request of a presidency /
institution) rather than on a pending proposal. These carry a separate CELEX
descriptor ``IE`` (``5{YYYY}IE{NNNN}``, sector 5 = EESC/CoR acts) and are
published in the OJ C series, registered in the EU Publications Office repository
(CELLAR).

This is a distinct, additive companion to EU/EESC-Opinions: the two use different
CELEX descriptors (``AE`` vs ``IE``) so there is no document overlap, and the
loader dedups on the CELEX ``_id`` in any case. The IE series spans 1973-present;
full text (OJ Formex/xHTML) is available from ~1994 onward (earlier acts are
metadata-only and are skipped by the ``< 200`` char guard).

Data flow
---------
1. Enumerate every EESC own-initiative/exploratory opinion via the public CELLAR
   SPARQL endpoint (CELEX matching ``^5[0-9]{4}IE``), retrieving CELEX + document
   date + English expression title. Paged with LIMIT/OFFSET (the IE corpus is
   well under the ~10 000-row SPARQL OFFSET ceiling).
...
# CELEX pattern for EESC own-initiative/exploratory opinions: 5{YYYY}IE{NNNN}
CELEX_RE = r"^5[0-9]{4}IE"

# Full OJ text only exists from the mid-1990s; earlier IE acts are metadata-only.
MIN_YEAR = 1994
...
def _celex_year(celex: str) -> Optional[int]:
    m = re.match(r"^5(\d{4})IE", celex)
    return int(m.group(1)) if m else None


class EESCInitiativeOpinionsScraper(BaseScraper):
    """Scraper for EESC own-initiative/exploratory opinions (IE) via CELLAR."""
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield RAW opinion dicts (with full text) for every EESC IE opinion."""
        offset = 0
        seen = set()
        while True:
    ...
    def normalize(self, raw: dict) -> dict:
        ...
        if not title:
            title = f"Opinion of the European Economic and Social Committee {celex}"
        date = raw.get("date")
        if date and not re.match(r"^\d{4}-\d{2}-\d{2}", str(date)):
            date = None
        return {
            "_id": celex,
            "_source": "EU/EESC-InitiativeOpinions",
            "_type": "doctrine",
...
def main():
    ...
    scraper = EESCInitiativeOpinionsScraper()
    ...
    if command in ("bootstrap", "bootstrap-fast"):
        ...
        print(f"SPARQL returned {len(page)} EESC IE opinions on page 1")
        if page:
            c = page[0]["celex"]
