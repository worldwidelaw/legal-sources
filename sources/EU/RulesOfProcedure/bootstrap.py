#!/usr/bin/env python3
"""
EU/RulesOfProcedure — Rules of procedure and institutional/procedural acts of the
EU institutions and bodies (CELEX **sector 3**, document class ``Q``).

The ``Q`` descriptor gathers the EU's **institutional and procedural** acts: the
**rules of procedure** of the EU institutions, courts and bodies (Court of Justice,
General Court, European Economic and Social Committee, Committee of the Regions, the
agencies' management boards, etc.), together with their amendments, **practice
directions** and **practice rules** issued by the Union courts, **inter-institutional
framework agreements** (e.g. the Framework Agreement on relations between the European
Parliament and the Commission), **codes of conduct**, and the internal decisions of
bodies such as the European Data Protection Supervisor on records/archives management.
They are published in the Official Journal and catalogued by the EU Publications Office
(CELLAR / EUR-Lex) under the sector-3 descriptor ``Q`` (CELEX ``3{YYYY}Q{NNNN}``).
...
and therefore never pulls the ``Q`` institutional/procedural acts — they carry the
dedicated rules-of-procedure / internal-agreement resource types, not DEC. No ``_id``
collision with any other descriptor.
...
2. Fetch the full text of each act from CELLAR via HTTP content negotiation:
   ...
   fleet-safe.
3. Normalize to the standard schema (doctrine — institutional/procedural acts).

Some ``Q`` CELEX carry a parenthesised sub-part (e.g. ``32020Q1012(01)``); the
CELEX is URL-encoded before content negotiation so those resolve correctly.

Same CELLAR recipe as EU/FrameworkDecisions and EU/CouncilResolutions, narrowed to
the sector-3 ``Q`` descriptor. Corrigenda (``…R(01)``) and metadata-only stubs
...
# CELEX pattern for sector-3 rules of procedure / institutional acts: 3{YYYY}Q{NNNN}[(NN)].
CELEX_RE = r"^3[0-9]{4}Q[0-9]"
...
class RulesOfProcedureScraper(BaseScraper):
    """Scraper for EU rules of procedure / institutional acts (CELEX sector-3 ``Q``) via CELLAR."""
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield RAW rules-of-procedure dicts (with full text), newest-first."""
        offset = 0
        seen = set()
        while True:
    ...
    def normalize(self, raw: dict) -> dict:
        ...
        if not title:
            title = f"Rules of Procedure / Institutional Act {celex}"
        date = raw.get("date")
        if date and not re.match(r"^\d{4}-\d{2}-\d{2}", str(date)):
            date = None
        return {
            "_id": celex,
            "_source": "EU/RulesOfProcedure",
            "_type": "doctrine",
...
def main():
    ...
    scraper = RulesOfProcedureScraper()
    ...
    if command in ("bootstrap", "bootstrap-fast"):
        ...
        print(f"SPARQL returned {len(page)} rules-of-procedure / institutional acts on page 1")
        if page:
            c = page[0]["celex"]
