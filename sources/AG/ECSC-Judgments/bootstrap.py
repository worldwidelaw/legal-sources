#!/usr/bin/env python3
"""
AG/ECSC-Judgments -- Antigua & Barbuda Court Judgments (ECSC)

Fetches court judgments from the Eastern Caribbean Supreme Court WordPress site
at judgments.eccourts.org. Uses the WP REST API with category 34
(Antigua & Barbuda).

Endpoints:
  - List: GET /wp-json/wp/v2/posts?categories=34&per_page=100&page={N}
...
logger = logging.getLogger("legal-data-hunter.AG.ECSC-Judgments")
...
class AGECSCJudgmentsScraper(BaseScraper):
    """Scraper for AG/ECSC-Judgments -- Antigua & Barbuda Court Judgments."""
    ...
    def _fetch_page(self, page: int) -> tuple:
        """Fetch a page of AG posts. Returns (posts, total_pages)."""
        params = {
            "categories": AG_CATEGORY,
            "per_page": PER_PAGE,
            "page": page,
    ...
    def normalize(self, raw: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "_id": f"AG/ECSC-Judgments/{wp_id}",
            "_source": "AG/ECSC-Judgments",
            "_type": "case_law",
            "_fetched_at": now,
            "title": title,
            "text": text,
...
if __name__ == "__main__":
    scraper = AGECSCJudgmentsScraper()
