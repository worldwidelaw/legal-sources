#!/usr/bin/env python3
"""
CH/AG-Legislation -- Kanton Aargau Systematische Gesetzessammlung (SAR)

Fetches Aargau cantonal legislation from the LexWork REST API.
Full text is returned as structured JSON — no PDF extraction needed.

Data source: https://gesetzessammlungen.ag.ch/
...
API_BASE = "https://gesetzessammlungen.ag.ch/api/de"
...
def extract_text_from_tree(node):
    ...
    html = node.get("html_content", {}).get("de", "")
    if html:
...
def normalize(law_info, text, meta):
    ...
    return {
        "_id": "CH/AG-Legislation/%s" % sn,
        "_source": "CH/AG-Legislation",
        "_type": "legislation",
        "_fetched_at": datetime.now(timezone.utc).isoformat(),
        "title": law_info["title"],
        "text": text,
        "date": meta.get("date", "") if meta else "",
        "url": "https://gesetzessammlungen.ag.ch/app/de/texts_of_law/%s" % sn,
...
def fetch_all(sample=False):
    """Fetch all Aargau cantonal legislation."""
    laws = get_law_index()
    if not laws:
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Aargau cantonal legislation fetcher")
