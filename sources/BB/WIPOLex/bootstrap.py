#!/usr/bin/env python3
"""
BB/WIPOLex -- Barbados legislation via the WIPO Lex database.

WIPO Lex (https://www.wipo.int/wipolex/) is WIPO's free, open gateway to the
intellectual-property and IP-related laws of ~200 jurisdictions. For Barbados it
holds the full text of core statutes -- copyright, patents, trademarks,
industrial designs, layout-designs of integrated circuits, plant breeders'
rights, the Constitution, the Trade Marks Act, the Income and Business Tax Act,
competition and consumer protection, their subsidiary regulations and related
commercial legislation -- as machine-readable PDF documents, in English
(Barbados's legislative language).

The global INTL/WIPOLex scraper only captures documents that expose inline HTML
full text and explicitly skips PDF-only laws. The bulk of Barbados's WIPO Lex
corpus is PDF-only, so this per-country source fills that gap by downloading and
extracting the CloudFront-signed PDFs.

Strategy:
  1. Fetch the WIPO Lex Barbados member profile
     (/wipolex/en/members/profile/BB). It is server-rendered HTML listing each
  ...
  2. For each detail page, extract the CloudFront-signed PDF download URL(s)
     (https://wipolex-res.wipo.int/edocs/lexdocs/laws/{lang}/bb/{code}.pdf).
     The signed `?last-modified=...` query string is required -- the bare URL
     returns an HTML error page. English is preferred (Barbados's legislative
     language).
...
# .../edocs/lexdocs/laws/en/bz/bz016en.pdf
LANG_RE = re.compile(r"/edocs/lexdocs/laws/([a-z]{2})/[a-z]{2}/")

# Language preference: English first (Barbados's legislative language).
...
def _parse_profile(html_text: str) -> List[Tuple[str, str, Optional[str]]]:
    """Return [(detail_id, title, iso_date)] for every Barbadosan legal text.
...
class WIPOLexBBScraper(BaseScraper):
    """Scraper for BB/WIPOLex."""
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        ...
        for detail_id, title, date in self._iter_docs():
            ...
            doc_id = f"BB_WIPOLEX_{detail_id}"
            text = None
            chosen_url = None
            language = None
            for url in urls:
    ...
    def fetch_updates(self, since: Any = None) -> Generator[Dict[str, Any], None, None]:
        # WIPO Lex has no incremental feed; re-scan the (small) BB corpus.
    ...
    def test(self) -> bool:
        logger.info("Testing BB/WIPOLex profile + detail + PDF extraction...")
        profile = _get(f"{HOST}{PROFILE_PATH}")
        if not profile:
            logger.error("Profile fetch failed")
            return False
        rows = _parse_profile(profile)
        logger.info("Parsed %d BB legal texts", len(rows))
        if len(rows) < 10:
            logger.error("Expected >=10 BB texts, got %d", len(rows))
            return False
