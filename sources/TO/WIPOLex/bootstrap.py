#!/usr/bin/env python3
"""
TO/WIPOLex -- Tonga legislation via the WIPO Lex database.

WIPO Lex (https://www.wipo.int/wipolex/) is WIPO's free, open gateway to the
intellectual-property and IP-related laws of ~200 jurisdictions. For Tonga it
holds the full text of core statutes — the Copyright Act, the Industrial
Property Act, the Constitution, and a range of related acts, regulations and
constitutional instruments — as machine-readable English PDF documents.

Tonga is one of the project's data-poor jurisdictions. PacLII (paclii.org) —
...
WIPO Lex is a reliably reachable source publishing machine-readable Tonga
statute text without authentication.

Strategy:
  1. Fetch the WIPO Lex Tonga member profile
     (/wipolex/en/members/profile/TO). It is server-rendered HTML listing each
  ...
  2. For each detail page, extract the CloudFront-signed PDF download URL(s)
     (https://wipolex-res.wipo.int/edocs/lexdocs/laws/{lang}/to/{code}.pdf).
...
logger = logging.getLogger("legal-data-hunter.TO.WIPOLex")

HOST = "https://www.wipo.int"
COUNTRY = "TO"
COUNTRY_NAME = "Tonga"
PROFILE_PATH = f"/wipolex/en/members/profile/{COUNTRY}"
SOURCE_ID = "TO/WIPOLex"
DELAY = 1.5
...
# .../edocs/lexdocs/laws/en/to/nr040en_1.pdf
LANG_RE = re.compile(r"/edocs/lexdocs/laws/([a-z]{2})/[a-z]{2}/")

# Language preference: English first (Tonga statutes are in English).
LANG_PRIORITY = {"en": 0}
...
def _parse_profile(html_text: str) -> List[Tuple[str, str, Optional[str]]]:
    """Return [(detail_id, title, iso_date)] for every Tonga legal text.
...
class WIPOLexTOScraper(BaseScraper):
    """Scraper for TO/WIPOLex."""
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        ...
        for detail_id, title, date in self._iter_docs():
            ...
            doc_id = f"TO_WIPOLEX_{detail_id}"
            text = None
            chosen_url = None
            language = None
            for url in urls:
    ...
    def fetch_updates(self, since: Any = None) -> Generator[Dict[str, Any], None, None]:
        # WIPO Lex has no incremental feed; re-scan the (small) TO corpus.
    ...
    def test(self) -> bool:
        logger.info("Testing TO/WIPOLex profile + detail + PDF extraction...")
        profile = _get(f"{HOST}{PROFILE_PATH}")
        if not profile:
            logger.error("Profile fetch failed")
            return False
        rows = _parse_profile(profile)
        logger.info("Parsed %d TO legal texts", len(rows))
        if len(rows) < 10:
            logger.error("Expected >=10 TO texts, got %d", len(rows))
            return False
