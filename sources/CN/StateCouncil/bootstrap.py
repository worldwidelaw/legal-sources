#!/usr/bin/env python3
"""
CN/StateCouncil -- China State Council Administrative Regulations (国务院行政法规)

Fetches administrative regulations from flk.npc.gov.cn.
Uses the same API as CN/NPC with flfgCodeId=[210, 215] to filter for
administrative regulations and their amendment/repeal decisions.
...
logger = logging.getLogger("legal-data-hunter.CN.StateCouncil")
...
def extract_text_from_docx(docx_bytes: bytes) -> str:
    """Extract text from DOCX using stdlib zipfile+xml."""
    try:
...
def search_admin_regs(page: int = 1, page_size: int = 20) -> tuple:
    """Search for administrative regulations. Returns (rows, total)."""
    payload = {
        "searchContent": "",
        "searchType": 2,
        "searchRange": 1,
        "flfgCodeId": [210, 215],  # 行政法规 + amendment/repeal decisions
        "zdjgCodeId": [],
        "sxx": [],
        "gbrq": [],
        "sxrq": [],
...
def fetch_sample(count: int = 15) -> list:
    ...
    logger.info("Searching for recent administrative regulations...")
    rows, total = search_admin_regs(page=1, page_size=count + 5)
    logger.info(f"Total administrative regulations available: {total:,}")
...
def fetch_all() -> Generator[dict, None, None]:
    """Fetch all administrative regulations with full text."""
    page = 1
    page_size = 50
    total_yielded = 0

    _, total = search_admin_regs(page=1, page_size=1)
    logger.info(f"Total administrative regulations: {total:,}")

    while True:
        rows, _ = search_admin_regs(page=page, page_size=page_size)
        if not rows:
            break
...
def test_api():
    ...
    logger.info("Testing NPC Administrative Regulations API...")

    try:
        rows, total = search_admin_regs(page=1, page_size=2)
        logger.info(f"Search OK - {total:,} total admin regs, got {len(rows)} results")
    except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description="CN/StateCouncil Admin Regs Fetcher")
