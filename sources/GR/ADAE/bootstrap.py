#!/usr/bin/env python3
"""
GR/ADAE -- Hellenic Authority for Communication Security and Privacy

Fetches ADAE decisions from the Diavgeia transparency portal API.

Strategy:
  - Uses the Diavgeia OpenData API filtered for ADAE organization (UID 22794)
  - Fetches regulatory acts (2.4.7.1 — Independent Authority Acts)
  - Full text extracted from PDF documents via pdfplumber
  - API supports pagination

Endpoints:
  - Search: https://diavgeia.gov.gr/luminapi/opendata/search?org=22794&type={type}
  - Document PDF: https://diavgeia.gov.gr/doc/{ada}

Data:
  - Communication privacy and security regulatory decisions
  - Inspection approvals, policy decisions, audit group formations
...
logger = logging.getLogger("legal-data-hunter.GR.ADAE")
...
DECISION_TYPES = [
    "2.4.7.1",  # ΠΡΑΞΗ ΑΝΕΞΑΡΤΗΤΗΣ ΑΡΧΗΣ (Independent Authority Act)
]


class ADAEScraper(BaseScraper):
    """
    Scraper for GR/ADAE -- Communication Security and Privacy Authority decisions.
    Country: GR
    URL: https://adae.gov.gr (data via Diavgeia)
    ...
    def _search_decisions(
        ...
        """Search for ADAE decisions of a specific type via the OpenData API."""
        try:
            ...
            logger.error(f"Failed to search ADAE decisions (type={decision_type}): {e}")
            return None
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield all ADAE decisions."""
        for decision_type in DECISION_TYPES:
            logger.info(f"Fetching ADAE decisions of type {decision_type}...")
            page = 0
            page_size = 50
    ...
    def fetch_updates(self, since: datetime) -> Generator[dict, None, None]:
        ...
        for decision_type in DECISION_TYPES:
            logger.info(f"Fetching ADAE updates of type {decision_type} since {since.isoformat()}...")
            page = 0
    ...
    def normalize(self, raw: dict) -> dict:
        ...
        title = subject or f"ADAE Decision {raw.get('ada')}"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="GR/ADAE Data Fetcher")
    ...
    if args.command == "test":
        print("Testing ADAE via Diavgeia API connection...")
        result = scraper._search_decisions("2.4.7.1", page=0, size=1)
        ...
        print(f"\nFetched {count} new ADAE decisions")
