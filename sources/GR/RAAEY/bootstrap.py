#!/usr/bin/env python3
"""
GR/RAAEY -- Greek Energy Regulatory Authority Data Fetcher

Fetches RAAEY (formerly RAE) decisions from the Diavgeia transparency portal API.

Strategy:
  - Uses the Diavgeia OpenData API filtered for RAAEY organization (UID 22793)
...
Endpoints:
  - Search: https://diavgeia.gov.gr/luminapi/opendata/search?org=22793&type={type}
  - Document PDF: https://diavgeia.gov.gr/doc/{ada}

Data:
  - Energy sector regulatory decisions
  - Licensing, tariff, and market regulation acts
...
logger = logging.getLogger("legal-data-hunter.GR.RAAEY")
...
class RAAEYScraper(BaseScraper):
    """
    Scraper for GR/RAAEY -- Greek Energy Regulatory Authority decisions.
    Country: GR
    URL: https://www.raaey.gr (data via Diavgeia)
    ...
    def _search_decisions(
        ...
        """Search for RAAEY decisions of a specific type via the OpenData API."""
        try:
            ...
            logger.error(f"Failed to search RAAEY decisions (type={decision_type}): {e}")
            return None
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield all RAAEY decisions."""
        for decision_type in DECISION_TYPES:
            logger.info(f"Fetching RAAEY decisions of type {decision_type}...")
            page = 0
            page_size = 50
    ...
    def fetch_updates(self, since: datetime) -> Generator[dict, None, None]:
        ...
        for decision_type in DECISION_TYPES:
            logger.info(f"Fetching RAAEY updates of type {decision_type} since {since.isoformat()}...")
            page = 0
    ...
    def normalize(self, raw: dict) -> dict:
        ...
        title = subject or f"RAAEY Decision {raw.get('ada')}"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="GR/RAAEY Data Fetcher")
    ...
    scraper = RAAEYScraper()

    if args.command == "test":
        print("Testing RAAEY via Diavgeia API connection...")
        ...
        print(f"\nFetched {count} new RAAEY decisions")
