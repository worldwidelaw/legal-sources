...
Fetches regulatory documents, directives, notices, and publications
from SECZ via WordPress REST API.

Strategy:
  - WP Media API: ~479 PDF attachments downloaded and text extracted
    via pdfplumber.
  - WP Posts API: ~604 posts with inline text (press releases, notices).
...
logger = logging.getLogger("legal-data-hunter.ZW.SECZ")

API_BASE = "https://seczim.co.zw/wp-json/wp/v2"
...
def classify_document(title: str) -> str:
    ...
    if "notice" in t:
        return "notice"
    if "regulation" in t or "regulations" in t or "statutory instrument" in t:
        return "regulation"
    if "annual report" in t or "industry report" in t or "newsletter" in t:
        return "report"
    if "rule" in t or "rules" in t:
        return "rule"
    if "decision" in t or "ruling" in t or "enforcement" in t:
        return "decision"
    if "policy" in t:
        return "policy"
    if "aml" in t or "anti-money" in t or "cft" in t:
        return "aml-guidance"
    return "publication"


class SECZScraper(BaseScraper):
    """
    Scraper for ZW/SECZ -- Securities and Exchange Commission of Zimbabwe.
    Country: ZW
    ...
    def _normalize_post(self, post: dict) -> Optional[dict]:
        ...
        link = post.get("link", f"https://seczim.co.zw/?p={post_id}")

        return {
            "_id": f"zw-secz-post-{post_id}",
            "_source": "ZW/SECZ",
            "_type": "doctrine",
            "_fetched_at": datetime.now(timezone.utc).isoformat(),
            "title": title,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ZW/SECZ -- Securities and Exchange Commission of Zimbabwe"
    )
