#!/usr/bin/env python3
"""
KI/WIPOLex -- Kiribati legislation via the WIPO Lex database.

WIPO Lex (https://www.wipo.int/wipolex/) is WIPO's free, open gateway to the
intellectual-property and IP-related laws of ~200 jurisdictions. For Kiribati
it holds the full text of core statutes — the Copyright Act 2018, the revised
Gilbert Islands ordinances (patents, trade marks, designs, merchandise marks),
the Penal Code and related acts — as machine-readable English PDF documents.

Kiribati is one of the project's data-poor jurisdictions: the native
legislation channels are blocked or empty. The government gazette publishes
only scanned image PDFs (no text layer), the Parliament "Acts of Kiribati"
page 404s, and the judiciary site redirects all judgments to PacLII — which
now serves a Cloudflare JS challenge to datacenter and residential IPs alike.
WIPO Lex is the one source publishing machine-readable Kiribati statute text
reachable without authentication.

Strategy:
  1. Fetch the WIPO Lex Kiribati member profile
     (/wipolex/en/members/profile/KI). It is server-rendered HTML listing each
  ...
  2. For each detail page, extract the CloudFront-signed PDF download URL(s)
     (https://wipolex-res.wipo.int/edocs/lexdocs/laws/{lang}/ki/{code}.pdf).
...
# .../edocs/lexdocs/laws/en/ki/ki029en.pdf
LANG_RE = re.compile(r"/edocs/lexdocs/laws/([a-z]{2})/[a-z]{2}/")

# Language preference: English first (Kiribati statutes are in English).
LANG_PRIORITY = {"en": 0}
...
def _parse_profile(html_text: str) -> List[Tuple[str, str, Optional[str]]]:
    """Return [(detail_id, title, iso_date)] for every Kiribati legal text.
...
class WIPOLexKIScraper(BaseScraper):
    """Scraper for KI/WIPOLex."""
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        ...
        for detail_id, title, date in self._iter_docs():
            ...
            doc_id = f"KI_WIPOLEX_{detail_id}"
            text = None
            chosen_url = None
            language = None
            for url in urls:
    ...
    def fetch_updates(self, since: Any = None) -> Generator[Dict[str, Any], None, None]:
        # WIPO Lex has no incremental feed; re-scan the (small) KI corpus.
    ...
    def test(self) -> bool:
        logger.info("Testing KI/WIPOLex profile + detail + PDF extraction...")
        profile = _get(f"{HOST}{PROFILE_PATH}")
        if not profile:
            logger.error("Profile fetch failed")
            return False
        rows = _parse_profile(profile)
        logger.info("Parsed %d KI legal texts", len(rows))
        if len(rows) < 10:
            logger.error("Expected >=10 KI texts, got %d", len(rows))
            return False
