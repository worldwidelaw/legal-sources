#!/usr/bin/env python3
"""
ZM/WIPOLex -- Republic of Zambia legislation via the WIPO Lex database.

WIPO Lex (https://www.wipo.int/wipolex/) is WIPO's free, open gateway to the
intellectual-property and IP-related laws of ~200 jurisdictions. For Zambia it
...
legislation -- as machine-readable PDF documents, in English (Zambia's
legislative language).

The global INTL/WIPOLex scraper only captures documents that expose inline HTML
full text and explicitly skips PDF-only laws. The bulk of Zambia's WIPO Lex
corpus is PDF-only, so this per-country source fills that gap by downloading and
extracting the CloudFront-signed PDFs.

Strategy:
  1. Fetch the WIPO Lex Zambia member profile
     (/wipolex/en/members/profile/ZM). It is server-rendered HTML listing each
  ...
  2. For each detail page, extract the CloudFront-signed PDF download URL(s)
     (https://wipolex-res.wipo.int/edocs/lexdocs/laws/{lang}/zm/{code}.pdf).
     The signed `?last-modified=...` query string is required -- the bare URL
     returns an HTML error page. English is preferred (Zambia's legislative
     language).
...
logger = logging.getLogger("legal-data-hunter.ZM.WIPOLex")

HOST = "https://www.wipo.int"
COUNTRY = "ZM"
COUNTRY_NAME = "Republic of Zambia"
PROFILE_PATH = f"/wipolex/en/members/profile/{COUNTRY}"
SOURCE_ID = "ZM/WIPOLex"
DELAY = 1.5
...
# Language preference: English first (Zambia's legislative language).
...
def _parse_profile(html_text: str) -> List[Tuple[str, str, Optional[str]]]:
    """Return [(detail_id, title, iso_date)] for every Zambian legal text.
...
class WIPOLexZMScraper(BaseScraper):
    """Scraper for ZM/WIPOLex."""
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        ...
        for detail_id, title, date in self._iter_docs():
            ...
            doc_id = f"ZM_WIPOLEX_{detail_id}"
            text = None
            chosen_url = None
            language = None
            for url in urls:
    ...
    def fetch_updates(self, since: Any = None) -> Generator[Dict[str, Any], None, None]:
        # WIPO Lex has no incremental feed; re-scan the (small) ZM corpus.
    ...
    def test(self) -> bool:
        logger.info("Testing ZM/WIPOLex profile + detail + PDF extraction...")
        profile = _get(f"{HOST}{PROFILE_PATH}")
        if not profile:
            logger.error("Profile fetch failed")
            return False
        rows = _parse_profile(profile)
        logger.info("Parsed %d ZM legal texts", len(rows))
        if len(rows) < 10:
            logger.error("Expected >=10 ZM texts, got %d", len(rows))
            return False
