#!/usr/bin/env python3
"""
BI/WIPOLex -- Burundi legislation via the WIPO Lex database.
...
Republic of Burundi it holds the full text of core statutes -- not only IP law
but broader instruments such as the codes and framework laws on industrial
property, copyright, investment and related commercial matters -- as
machine-readable PDF documents, predominantly in French (Burundi's main
legislative language alongside Kirundi).

Burundi has no comprehensive open legislation API or bulk download reachable
from outside the country; the official gazette / government portals are
unreliable from datacenter IPs and largely serve scanned PDFs. WIPO Lex
publishes machine-readable Burundi statute text without authentication and is
reachable from datacenter IPs, making it a stable full-text source of the
country's law.

Strategy:
  1. Fetch the WIPO Lex Burundi member profile
     (/wipolex/en/members/profile/BI). It is server-rendered HTML listing each
  ...
  2. For each detail page, extract the CloudFront-signed PDF download URL(s)
     (https://wipolex-res.wipo.int/edocs/lexdocs/laws/{lang}/bi/{code}.pdf).
     The signed `?last-modified=...` query string is required -- the bare URL
     returns an HTML error page. French is preferred (Burundi's main legislative
     language), then English, then Arabic.
...
# .../edocs/lexdocs/laws/fr/bi/bi001fr.pdf
LANG_RE = re.compile(r"/edocs/lexdocs/laws/([a-z]{2})/[a-z]{2}/")

# Language preference: French first (Burundi's main legislative language), then
# English, then Arabic.
...
def _parse_profile(html_text: str) -> List[Tuple[str, str, Optional[str]]]:
    """Return [(detail_id, title, iso_date)] for every Burundi legal text.
...
class WIPOLexBIScraper(BaseScraper):
    """Scraper for BI/WIPOLex."""
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        ...
        for detail_id, title, date in self._iter_docs():
            ...
            doc_id = f"BI_WIPOLEX_{detail_id}"
            text = None
            chosen_url = None
            language = None
            for url in urls:
    ...
    def fetch_updates(self, since: Any = None) -> Generator[Dict[str, Any], None, None]:
        # WIPO Lex has no incremental feed; re-scan the (small) BI corpus.
    ...
    def test(self) -> bool:
        logger.info("Testing BI/WIPOLex profile + detail + PDF extraction...")
        profile = _get(f"{HOST}{PROFILE_PATH}")
        if not profile:
            logger.error("Profile fetch failed")
            return False
        rows = _parse_profile(profile)
        logger.info("Parsed %d BI legal texts", len(rows))
        if len(rows) < 10:
            logger.error("Expected >=10 BI texts, got %d", len(rows))
            return False
