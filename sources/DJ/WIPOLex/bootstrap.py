#!/usr/bin/env python3
"""
DJ/WIPOLex -- Djibouti legislation via the WIPO Lex database.

WIPO Lex (https://www.wipo.int/wipolex/) is WIPO's free, open gateway to the
intellectual-property and IP-related laws of ~200 jurisdictions. For Djibouti it
holds the full text of core statutes -- the codes and laws on industrial
property, copyright, commerce, investment and related acts -- as
machine-readable PDF documents, almost all in French (Djibouti's legislative
language), with a few in Arabic or English.

Djibouti has no comprehensive open legislation API reachable from outside the
country (the official Journal Officiel is covered by DJ/JournalOfficiel, but its
historical depth is limited). WIPO Lex publishes machine-readable Djiboutian
statute text without authentication, complementing the existing DJ sources.

Strategy:
  1. Fetch the WIPO Lex Djibouti member profile
     (/wipolex/en/members/profile/DJ). It is server-rendered HTML listing each
  ...
  2. For each detail page, extract the CloudFront-signed PDF download URL(s)
     (https://wipolex-res.wipo.int/edocs/lexdocs/laws/{lang}/dj/{code}.pdf).
     The signed `?last-modified=...` query string is required -- the bare URL
     returns an HTML error page. French is preferred, then English, then Arabic.
...
# .../edocs/lexdocs/laws/fr/dj/dj010fr_1.pdf
LANG_RE = re.compile(r"/edocs/lexdocs/laws/([a-z]{2})/[a-z]{2}/")

# Language preference: French first (Djibouti's legislative language), then
# English, then Arabic.
...
def _parse_profile(html_text: str) -> List[Tuple[str, str, Optional[str]]]:
    """Return [(detail_id, title, iso_date)] for every Djibouti legal text.
...
class WIPOLexDJScraper(BaseScraper):
    """Scraper for DJ/WIPOLex."""
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        ...
        for detail_id, title, date in self._iter_docs():
            ...
            doc_id = f"DJ_WIPOLEX_{detail_id}"
            text = None
            chosen_url = None
            language = None
            for url in urls:
    ...
    def fetch_updates(self, since: Any = None) -> Generator[Dict[str, Any], None, None]:
        # WIPO Lex has no incremental feed; re-scan the (small) DJ corpus.
    ...
    def test(self) -> bool:
        logger.info("Testing DJ/WIPOLex profile + detail + PDF extraction...")
        profile = _get(f"{HOST}{PROFILE_PATH}")
        if not profile:
            logger.error("Profile fetch failed")
            return False
        rows = _parse_profile(profile)
        logger.info("Parsed %d DJ legal texts", len(rows))
        if len(rows) < 10:
            logger.error("Expected >=10 DJ texts, got %d", len(rows))
            return False
