#!/usr/bin/env python3
"""
ES/Murcia -- Murcia Regional Legislation (BORM)

Fetches legislation from the Boletín Oficial de la Región de Murcia via:
  1. BOE ELI sitemaps to discover all Murcia documents (es-mc prefix)
...
logger = logging.getLogger("legal-data-hunter.ES.murcia")
...
class MurciaScraper(BaseScraper):
    """Scraper for ES/Murcia -- BOE ELI + Open Data API for full text."""
    ...
    def _discover_eli_uris(self) -> List[str]:
        """Discover all Murcia ELI URIs from BOE sitemaps."""
        uris = []
        for i in range(3):
            ...
            uris.extend(matches)
            logger.info(f"  Found {len(matches)} Murcia entries in sitemap{i}")
        logger.info(f"Total Murcia ELI URIs discovered: {len(uris)}")
        return uris
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield all legislation from BOE ELI sitemaps for Murcia."""
    ...
    def test_connection(self):
        ...
        print("Testing BOE ELI / Open Data API for Murcia...")

        print("\n1. Testing ELI sitemap discovery...")
        uris = self._discover_eli_uris()
        print(f"   Found {len(uris)} Murcia ELI URIs")
