...
Fetches legislation from the Diario Oficial de Galicia via:
  1. BOE ELI sitemaps to discover all Galician documents (es-ga prefix)
...
logger = logging.getLogger("legal-data-hunter.ES.galicia")
...
class GaliciaScraper(BaseScraper):
    """Scraper for ES/Galicia -- BOE ELI + Open Data API for full text."""
    ...
    def _discover_eli_uris(self) -> List[str]:
        """Discover all Galicia ELI URIs from BOE sitemaps."""
        uris = []
        for i in range(3):
            ...
            if not content:
                continue
            # Extract all es-ga URIs
            matches = re.findall(
                r'<loc>(https://www\.boe\.es/eli/es-ga/[^<]+)</loc>',
                content
            )
            uris.extend(matches)
            logger.info(f"  Found {len(matches)} Galicia entries in sitemap{i}")
        logger.info(f"Total Galicia ELI URIs discovered: {len(uris)}")
        return uris

    def _parse_eli_uri(self, uri: str) -> Optional[Dict]:
        """Parse an ELI URI into components.

        URI format: https://www.boe.es/eli/es-ga/{type}/{year}/{month}/{day}/{number}
        """
        m = re.match(
            r'https://www\.boe\.es/eli/es-ga/([a-z]+)/(\d{4})/(\d{1,2})/(\d{1,2})/(\d+)',
            uri
        )
        if not m:
            return None
    ...
    def _get_boe_id(self, eli_url: str) -> Optional[str]:
        ...
        if not page:
            return None
        # Look for id_local meta tag which has the BOE-A ID
        m = re.search(
        ...
        if m:
            return m.group(1)
        # Fallback: look in title or meta description
    ...
    def _extract_text_from_xml(self, xml_content: str) -> str:
        ...
        # Extract all text within <texto> response
        texto_match = re.search(r'<texto>(.*)</texto>', xml_content, re.S)
        if not texto_match:
            # Try extracting from individual bloque elements
    ...
    def _fetch_document(self, eli_info: Dict) -> Optional[dict]:
        ...
        # Step 1: Get BOE-A ID from ELI page
        ...
        # Build title from metadata or construct from ELI info
        title = ""
        rango = ""
        fecha_disposicion = ""
        departamento = ""
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield all legislation from BOE ELI sitemaps for Galicia."""
    ...
    def test_connection(self):
        ...
        print("Testing BOE ELI / Open Data API for Galicia...")

        print("\n1. Testing ELI sitemap discovery...")
        uris = self._discover_eli_uris()
        print(f"   Found {len(uris)} Galicia ELI URIs")
        ...
        test_uri = uris[0]
        eli_info = self._parse_eli_uri(test_uri)
