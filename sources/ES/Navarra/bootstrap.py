#!/usr/bin/env python3
"""
ES/Navarra -- Navarra Regional Legislation (BON)

Fetches legislation from the Boletín Oficial de Navarra via:
  1. BOE ELI sitemaps to discover all Navarra documents (es-nc prefix)
...
logger = logging.getLogger("legal-data-hunter.ES.navarra")
...
class NavarraScraper(BaseScraper):
    """Scraper for ES/Navarra -- BOE ELI + Open Data API for full text."""
    ...
    def _discover_eli_uris(self) -> List[str]:
        """Discover all Navarra ELI URIs from BOE sitemaps."""
        uris = []
        for i in range(3):
            ...
            uris.extend(matches)
            logger.info(f"  Found {len(matches)} Navarra entries in sitemap{i}")
        logger.info(f"Total Navarra ELI URIs discovered: {len(uris)}")
        return uris
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield all legislation from BOE ELI sitemaps for Navarra."""
    ...
    def test_connection(self):
        ...
        print("Testing BOE ELI / Open Data API for Navarra...")

        print("\n1. Testing ELI sitemap discovery...")
        uris = self._discover_eli_uris()
        print(f"   Found {len(uris)} Navarra ELI URIs")
