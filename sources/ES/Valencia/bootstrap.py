#!/usr/bin/env python3
"""
ES/Valencia -- Valencia Regional Legislation (DOGV)

Fetches legislation from the Diari Oficial de la Generalitat Valenciana via:
  1. BOE ELI sitemaps to discover all Valencia documents (es-vc prefix)
...
logger = logging.getLogger("legal-data-hunter.ES.valencia")
...
class ValenciaScraper(BaseScraper):
    """Scraper for ES/Valencia -- BOE ELI + Open Data API for full text."""
    ...
    def _discover_eli_uris(self) -> List[str]:
        """Discover all Valencia ELI URIs from BOE sitemaps."""
        uris = []
        for i in range(3):
            ...
            uris.extend(matches)
            logger.info(f"  Found {len(matches)} Valencia entries in sitemap{i}")
        logger.info(f"Total Valencia ELI URIs discovered: {len(uris)}")
        return uris
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield all legislation from BOE ELI sitemaps for Valencia."""
    ...
    def test_connection(self):
        ...
        print("Testing BOE ELI / Open Data API for Valencia...")

        print("\n1. Testing ELI sitemap discovery...")
        uris = self._discover_eli_uris()
        print(f"   Found {len(uris)} Valencia ELI URIs")
