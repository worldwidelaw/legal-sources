#!/usr/bin/env python3
"""
BZ/SupremeCourt - Belize Supreme Court (High Court) Judgments

Fetches Supreme Court judgments from the Judiciary of Belize website.
Covers criminal cases (Indictments) from 1977-2022 and civil/mixed cases
from 1972-2022. Published as PDFs on the official judiciary.bz WordPress site.

Data source: https://judiciary.bz/judgements2/
...
SOURCE_ID = "BZ/SupremeCourt"
SAMPLE_DIR = Path(__file__).parent / "sample"


class SupremeCourtFetcher:
    """Fetcher for Belize Supreme Court judgments."""
    ...
    def get_year_pages(self) -> List[Tuple[str, str, str]]:
        """Parse the main judgements2 page to get all year page URLs.

        Returns list of (url, year_label, case_type) tuples.
        The page has two sections: Criminal (first set) and Civil (second set).
        """
        url = f"{BASE_URL}/judgements2/"
        try:
        ...
        # Detect split where a year appears for the second time
        seen_years = set()
    ...
    def get_judgments_from_page(self, page_url: str) -> List[Dict[str, str]]:
        ...
        for link in soup.find_all('a', href=True):
            ...
            if '.pdf' not in href.lower():
                continue
            # Only wp-content links are live
    ...
    def parse_case_metadata(self, title: str, pdf_url: str, case_type: str, year_label: str) -> Dict[str, Any]:
        """Extract structured metadata from case title."""
        # Detect case sub-type from title
        title_lower = title.lower()
        if 'indictment' in title_lower:
            sub_type = "criminal_indictment"
        elif 'civil appeal' in title_lower:
            sub_type = "civil_appeal"
        elif 'criminal appeal' in title_lower:
            sub_type = "criminal_appeal"
        elif 'supreme court action' in title_lower:
            sub_type = "civil_action"
        elif 'inferior' in title_lower:
            sub_type = "inferior_appeal"
        elif 'claim' in title_lower:
            sub_type = "civil_claim"
        else:
            sub_type = case_type

        # Extract case number and year
        case_match = re.search(
            r'(?:No\.?|Action)\s*(?:C?)(\d+)\s*(?:of\s*)?(\d{4})',
            title, re.IGNORECASE
        )
        case_number = case_match.group(1) if case_match else None
        case_year = case_match.group(2) if case_match else None

        # Extract parties
        parties_match = re.search(r'[-–—]\s*(.+)', title)
        parties = parties_match.group(1).strip() if parties_match else None
        if not parties:
            v_match = re.search(r'(.+?)\s+v[s]?\s+(.+)', title, re.IGNORECASE)
            if v_match:
                parties = f"{v_match.group(1).strip()} v {v_match.group(2).strip()}"

        date = f"{year_label}-01-01"
    ...
    def normalize(self, title: str, pdf_url: str, text: str, case_type: str, year_label: str) -> Dict[str, Any]:
        """Normalize a judgment record into the standard schema."""
        # Clean filename-derived titles
        if re.match(r'^[\w]+-[\w]+-', title) and ' ' not in title:
            title = re.sub(r'-+', ' ', title)

        meta = self.parse_case_metadata(title, pdf_url, case_type, year_label)
    ...
    def fetch_all(self) -> Iterator[Dict[str, Any]]:
        ...
        for page_url, year_label, case_type in year_pages:
            logger.info(f"Fetching {case_type} {year_label}: {page_url}")
            ...
            for j in judgments:
                ...
                if text:
                    yield self.normalize(j["title"], j["url"], text, case_type, year_label)
                else:
    ...
    def fetch_sample(self, max_records: int = 15) -> List[Dict[str, Any]]:
        ...
        # Pick pages known to have content (civil section has most PDFs)
        criminal_pages = [(u, y, t) for u, y, t in year_pages if t == "criminal"]
        civil_pages = [(u, y, t) for u, y, t in year_pages if t == "civil"]

        # Sample from civil (richer) and criminal sections
        sample_pages = []
        for p in civil_pages:
            if p[1] in ("2017", "2016", "2015", "2018"):
                sample_pages.append(p)
                if len(sample_pages) >= 2:
                    break
        for p in criminal_pages:
            if p[1] in ("2018", "2017", "2016", "2020"):
                sample_pages.append(p)
                if len(sample_pages) >= 4:
                    break

        for page_url, year_label, case_type in sample_pages:
            if len(records) >= max_records:
                break
            logger.info(f"Sampling from {case_type} {year_label}: {page_url}")
            judgments = self.get_judgments_from_page(page_url)
            ...
            for j in judgments[:5]:
                ...
                text = self.download_and_extract(j["url"])
                if text:
                    record = self.normalize(j["title"], j["url"], text, case_type, year_label)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="BZ/SupremeCourt - Belize Supreme Court Judgments")
