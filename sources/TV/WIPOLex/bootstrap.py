#!/usr/bin/env python3
"""
TV/WIPOLex -- Tuvalu legislation via the WIPO Lex database.

WIPO Lex (https://www.wipo.int/wipolex/) is WIPO's free, open gateway to the
intellectual-property and IP-related laws of ~200 jurisdictions. For Tuvalu it
holds the full text of core statutes — the Copyright Act (Cap. 40.24), the
Trademarks / Patents / United Kingdom designs ordinances, and a range of
related acts and constitutional instruments — as machine-readable English PDF
documents.

Tuvalu is one of the project's data-poor jurisdictions: the native legislation
channels are blocked or empty. The dedicated tuvalu-legislation.tv site serves
only a 294-byte blank shell, and PacLII (paclii.org) — the historical Pacific
legislation gateway — now returns HTTP 410 Gone behind a Cloudflare/ALTCHA
anti-bot challenge to datacenter and residential IPs alike. WIPO Lex is the one
source publishing machine-readable Tuvalu statute text reachable without
authentication.

Strategy:
  1. Fetch the WIPO Lex Tuvalu member profile
     (/wipolex/en/members/profile/TV). It is server-rendered HTML listing each
  ...
  2. For each detail page, extract the CloudFront-signed PDF download URL(s)
     (https://wipolex-res.wipo.int/edocs/lexdocs/laws/{lang}/tv/{code}.pdf).
...
logger = logging.getLogger("legal-data-hunter.TV.WIPOLex")

HOST = "https://www.wipo.int"
COUNTRY = "TV"
COUNTRY_NAME = "Tuvalu"
PROFILE_PATH = f"/wipolex/en/members/profile/{COUNTRY}"
SOURCE_ID = "TV/WIPOLex"
DELAY = 1.5
...
# .../edocs/lexdocs/laws/en/tv/tv022en_1.pdf
LANG_RE = re.compile(r"/edocs/lexdocs/laws/([a-z]{2})/[a-z]{2}/")

# Language preference: English first (Tuvalu statutes are in English).
LANG_PRIORITY = {"en": 0}
...
def _parse_profile(html_text: str) -> List[Tuple[str, str, Optional[str]]]:
    """Return [(detail_id, title, iso_date)] for every Tuvalu legal text.
...
class WIPOLexTVScraper(BaseScraper):
    """Scraper for TV/WIPOLex."""
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        ...
        for detail_id, title, date in self._iter_docs():
            ...
            doc_id = f"TV_WIPOLEX_{detail_id}"
            text = None
            chosen_url = None
            language = None
            for url in urls:
    ...
    def fetch_updates(self, since: Any = None) -> Generator[Dict[str, Any], None, None]:
        # WIPO Lex has no incremental feed; re-scan the (small) TV corpus.
    ...
    def test(self) -> bool:
        logger.info("Testing TV/WIPOLex profile + detail + PDF extraction...")
        profile = _get(f"{HOST}{PROFILE_PATH}")
        if not profile:
            logger.error("Profile fetch failed")
            return False
        rows = _parse_profile(profile)
        logger.info("Parsed %d TV legal texts", len(rows))
        if len(rows) < 10:
            logger.error("Expected >=10 TV texts, got %d", len(rows))
            return False
