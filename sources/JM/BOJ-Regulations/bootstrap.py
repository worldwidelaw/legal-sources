#!/usr/bin/env python3
"""
JM/BOJ-Regulations — Bank of Jamaica

Fetches legislation (acts, regulations, codes of conduct) and supervisory
guidance (standards of sound practice, consultation papers, AML guidance)
from the Bank of Jamaica website.

Strategy:
  1. Scrape the legislation and supervisory guidance pages for PDF links
...
logger = logging.getLogger("legal-data-hunter.JM.BOJ-Regulations")

BASE_URL = "https://boj.org.jm"
SOURCE_ID = "JM/BOJ-Regulations"

# Section pages to scrape for PDF links
SECTIONS = [
    {
        "url": "/about-boj/legislation/",
        "doc_type": "legislation",
        "category": "Legislation",
    },
    {
        "url": "/core-functions/financial-system/supervisory-guidance-notes-reporting-requirements/",
        "doc_type": "guideline",
        "category": "Supervisory Guidance",
    },
    {
        "url": "/core-functions/financial-system/boj-anti-money-laundering-aml/",
        "doc_type": "guideline",
        "category": "AML/CFT",
    },
    {
        "url": "/core-functions/financial-system/microcredit-regulation/",
        "doc_type": "legislation",
        "category": "Microcredit Regulation",
    },
]
...
class BOJRegulationsScraper(BaseScraper):
    ...
    def _scrape_pdf_links(self, page_path: str) -> list[dict]:
        ...
        for href, label in matches:
            ...
            # Skip non-document files (images, spreadsheets)
            lower_url = norm_url.lower()
            if any(lower_url.endswith(ext) for ext in [".xlsx", ".xls", ".png", ".jpg"]):
                continue

            # Clean label
            clean_label = re.sub(r"<[^>]+>", "", label).strip()
            if not clean_label:
                clean_label = Path(unquote(norm_url)).stem.replace("-", " ").replace("_", " ")
    ...
    def _normalize_record(self, pdf_info: dict, section: dict) -> Optional[dict]:
    ...
    def fetch_all(self) -> Generator[dict, None, None]:
        """Yield all BOJ regulatory documents."""
        global_seen = set()
        for section in SECTIONS:
            pdf_links = self._scrape_pdf_links(section["url"])
            for pdf_info in pdf_links:
                # Cross-section dedup by normalized URL
                if pdf_info["norm_url"] in global_seen:
                    logger.debug("Skipping duplicate: %s", pdf_info["norm_url"])
                    continue
...
def main():
    ...
    parser = argparse.ArgumentParser(description="JM/BOJ-Regulations scraper")
    parser.add_argument("command", choices=["bootstrap", "update", "test-api"])
    parser.add_argument("--sample", action="store_true", help="Fetch only sample records")
    parser.add_argument("--full", action="store_true", help="Fetch all records")
    args = parser.parse_args()

    scraper = BOJRegulationsScraper()
