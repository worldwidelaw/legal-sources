#!/usr/bin/env python3
"""
NL/CVDR -- Dutch Decentralized Regulations (Centrale Voorziening Decentrale Regelgeving)

Fetches the full-text corpus of Dutch decentralized (local) regulations from the
CVDR via the official SRU 1.2 API. Covers regulations enacted by:
  - Gemeenten (municipalities)       ~299,000 regulations
  - Provincies (provinces)           ~16,900 regulations
  - Waterschappen (water boards)     ~6,800 regulations

This is the decentralized counterpart to NL/wetten.overheid.nl (national law).
It is DISTINCT from the Caribbean CVDR subsets already covered by AW/Legislation,
CW/Legislation and SX/Legislation (those filter on koninkrijksdeel; this source
filters on the Dutch organisatietype and thereby excludes them).

Strategy:
  1. SRU searchRetrieve, scoped to Dutch org types, to get metadata + XML URLs
  2. Download full-text XML from the officiele-overheidspublicaties repository
  3. Extract plain text from the CVDR XML structure (born-digital, no OCR)
...
logger = logging.getLogger("legal-data-hunter.NL.CVDR")

SOURCE_ID = "NL/CVDR"

# Scope to Dutch decentralized government types. This cleanly excludes the
# Caribbean koninkrijksdeel subsets (Aruba/Curacao/Sint Maarten) which have
# their own dedicated sources.
JURISDICTION_QUERY = (
    'organisatietype=Gemeente OR organisatietype=Provincie '
    'OR organisatietype=Waterschap'
)
...
class NLCVDRScraper(BaseScraper):
    """Scraper for NL/CVDR -- Dutch decentralized regulations via CVDR SRU."""
    ...
    def _parse_record(self, record: ET.Element) -> Optional[dict]:
        ...
        try:
            ...
            creator = _get_text(meta, ["creator"])
            ...
            if enriched is not None:
                ...
                organisatietype = _get_text(enriched, ["organisatietype"])
    ...
    def _download_full_text(self, xml_url: str) -> str:
        ...
        # Extract the regulation body. Prefer the <lichaam> (body) subtree so that
        # metadata boilerplate outside it does not leak into the text.
        body = None
    ...
    def normalize(self, raw: dict) -> dict:
        ...
        return {
            ...
            "organisatietype": raw.get("organisatietype", ""),
        }
...
if __name__ == "__main__":
    ...
    if command == "test":
        ...
        print(f"Connection OK. Dutch CVDR records: {num.text if num is not None else '?'}")
        records = root.findall(".//srw:record", NS)
        if records:
