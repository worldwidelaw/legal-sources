#!/usr/bin/env python3
"""
KM/WIPOLex -- Comoros legislation via the WIPO Lex database.

WIPO Lex (https://www.wipo.int/wipolex/) is WIPO's free, open gateway to the
intellectual-property and IP-related laws of ~200 jurisdictions. For the Comoros it
...
machine-readable PDF documents, almost all in French (Comoros's official
languages alongside Arabic), with a few in English.
...
Strategy:
  1. Fetch the WIPO Lex Comoros member profile
     (/wipolex/en/members/profile/KM). It is server-rendered HTML listing each
...
# .../edocs/lexdocs/laws/fr/km/km###fr.pdf
LANG_RE = re.compile(r"/edocs/lexdocs/laws/([a-z]{2})/[a-z]{2}/")

# Language preference: French first (a Comoros official language), then
# English, then Arabic.
...
def _parse_profile(html_text: str) -> List[Tuple[str, str, Optional[str]]]:
    """Return [(detail_id, title, iso_date)] for every Comoros legal text.
...
class WIPOLexKMScraper(BaseScraper):
    """Scraper for KM/WIPOLex."""
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        ...
        for detail_id, title, date in self._iter_docs():
            ...
            doc_id = f"KM_WIPOLEX_{detail_id}"
            text = None
            chosen_url = None
            language = None
            for url in urls:
    ...
    def fetch_updates(self, since: Any = None) -> Generator[Dict[str, Any], None, None]:
        # WIPO Lex has no incremental feed; re-scan the (small) KM corpus.
    ...
    def test(self) -> bool:
        logger.info("Testing KM/WIPOLex profile + detail + PDF extraction...")
        profile = _get(f"{HOST}{PROFILE_PATH}")
        if not profile:
            logger.error("Profile fetch failed")
            return False
        rows = _parse_profile(profile)
        logger.info("Parsed %d KM legal texts", len(rows))
        if len(rows) < 10:
            logger.error("Expected >=10 KM texts, got %d", len(rows))
            return False
