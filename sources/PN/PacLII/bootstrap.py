#!/usr/bin/env python3
"""
PN/PacLII -- Pitcairn Islands Legislation & Court Decisions (PacLII) Fetcher

Fetches case law and legislation from the Pacific Islands Legal Information
Institute (PacLII) for Pitcairn Islands.
...
logger = logging.getLogger("legal-data-hunter.PN.PacLII")

BASE_URL = "https://www.paclii.org"

COURTS = [
    {"code": "PNPC", "name": "Privy Council - Pitcairn", "path": "/pn/cases/PNPC/"},
    {"code": "PNCA", "name": "Court of Appeal", "path": "/pn/cases/PNCA/"},
    {"code": "PNSC", "name": "Supreme Court", "path": "/pn/cases/PNSC/"},
]

LEGIS_DATABASES = [
    {"type": "consol_act", "name": "Consolidated Legislation", "path": "/pn/legis/consol_act/"},
    {"type": "num_act", "name": "Sessional Legislation", "path": "/pn/legis/num_act/"},
    {"type": "sub_leg", "name": "Subsidiary Legislation", "path": "/pn/legis/sub_leg/"},
    {"type": "hist_act", "name": "Historical Consolidated Legislation", "path": "/pn/legis/hist_act/"},
]

RULES_DATABASES = [
    {"type": "ct_rules", "name": "Court Rules", "path": "/pn/rules/ct_rules/"},
]
...
class PitcairnPacLIIScraper(BaseScraper):
    """Scraper for PN/PacLII -- Pitcairn Islands legal data."""
    ...
    def _parse_case_date(self, title: str) -> Optional[str]:
        """Extract date from case title like '[2024] PNSC 3 (29 January 2024)'."""
    ...
    def _parse_legis_date(self, title: str) -> Optional[str]:
        """Extract year from legislation title like 'Arms Ordinance 1974'."""
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        """Fetch all Pitcairn Islands documents from PacLII."""
        count = 0
    ...
    def test(self) -> bool:
        ...
        resp = self._request(BASE_URL + "/pn/cases/PNSC/")
        if resp is None:
            logger.error("Cannot reach PacLII Pitcairn Supreme Court")
            return False

        logger.info("PacLII Pitcairn Supreme Court page OK")

        doc = self._extract_document(BASE_URL + "/pn/cases/PNSC/2024/1.html")
        if doc:
            logger.info(f"Case OK: {doc['title'][:60]} ({len(doc['text'])} chars)")
        else:
            doc = self._extract_document(BASE_URL + "/pn/cases/PNSC/2023/1.html")
            if doc:
                logger.info(f"Case OK: {doc['title'][:60]} ({len(doc['text'])} chars)")
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="PN/PacLII data fetcher")
    parser.add_argument(
        "command",
