#!/usr/bin/env python3
"""
BT/WIPOLex -- Bhutan legislation via the WIPO Lex database.
...
Kingdom of Bhutan it holds the full text of core statutes -- the copyright and
industrial-property acts, civil and criminal codes, companies, contract, customs
and related commercial legislation -- as machine-readable PDF documents in
English (Bhutan's statutes are enacted in English alongside Dzongkha).
...
or rate-limit datacenter traffic. WIPO Lex publishes machine-readable Bhutanese
statute text in English without authentication and is a stable full-text source.

Strategy:
  1. Fetch the WIPO Lex Bhutan member profile
     (/wipolex/en/members/profile/BT). It is server-rendered HTML listing each
  ...
  2. For each detail page, extract the CloudFront-signed PDF download URL(s)
     (https://wipolex-res.wipo.int/edocs/lexdocs/laws/{lang}/bt/{code}.pdf).
     The signed `?last-modified=...` query string is required -- the bare URL
     returns an HTML error page. English is preferred (Bhutan's
     enacted statute language), then French.
...
logger = logging.getLogger("legal-data-hunter.BT.WIPOLex")

HOST = "https://www.wipo.int"
COUNTRY = "BT"
COUNTRY_NAME = "Bhutan"
PROFILE_PATH = f"/wipolex/en/members/profile/{COUNTRY}"
SOURCE_ID = "BT/WIPOLex"
DELAY = 1.5
...
# .../edocs/lexdocs/laws/en/bt/bt010en.pdf
LANG_RE = re.compile(r"/edocs/lexdocs/laws/([a-z]{2})/[a-z]{2}/")

# Language preference: English first (Bhutan's enacted statute language).
...
def _parse_profile(html_text: str) -> List[Tuple[str, str, Optional[str]]]:
    """Return [(detail_id, title, iso_date)] for every Bhutanese legal text.
...
class WIPOLexBTScraper(BaseScraper):
    """Scraper for BT/WIPOLex."""
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        ...
        for detail_id, title, date in self._iter_docs():
            ...
            doc_id = f"BT_WIPOLEX_{detail_id}"
            text = None
            chosen_url = None
            language = None
            for url in urls:
    ...
    def fetch_updates(self, since: Any = None) -> Generator[Dict[str, Any], None, None]:
        # WIPO Lex has no incremental feed; re-scan the (small) BT corpus.
    ...
    def test(self) -> bool:
        logger.info("Testing BT/WIPOLex profile + detail + PDF extraction...")
        profile = _get(f"{HOST}{PROFILE_PATH}")
        if not profile:
            logger.error("Profile fetch failed")
            return False
        rows = _parse_profile(profile)
        logger.info("Parsed %d BT legal texts", len(rows))
        if len(rows) < 10:
            logger.error("Expected >=10 BT texts, got %d", len(rows))
            return False
