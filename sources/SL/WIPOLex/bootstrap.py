#!/usr/bin/env python3
"""
SL/WIPOLex -- Sierra Leone legislation via the WIPO Lex database.
...
of Sierra Leone it holds the full text of core statutes -- the acts and regulations on
...
PDF documents in English (Sierra Leone's legislative language).

Sierra Leone has no comprehensive open legislation API or bulk download reachable from
outside the country; the consolidated statute collection lives behind the
SierraLII platform (a Laws.Africa / LII site whose document pages block datacenter
IPs and carry a CC BY-NC restriction). WIPO Lex publishes machine-readable Sierra Leone
statute text without authentication and is a stable, commercially-usable full-text
source of the country's law.

Strategy:
  1. Fetch the WIPO Lex Sierra Leone member profile
     (/wipolex/en/members/profile/SL). It is server-rendered HTML listing each
  ...
  2. For each detail page, extract the CloudFront-signed PDF download URL(s)
     (https://wipolex-res.wipo.int/edocs/lexdocs/laws/{lang}/sl/{code}.pdf).
     The signed `?last-modified=...` query string is required -- the bare URL
     returns an HTML error page. English is preferred (Sierra Leone's legislative
     language).
...
# .../edocs/lexdocs/laws/en/sl/sl001en.pdf
LANG_RE = re.compile(r"/edocs/lexdocs/laws/([a-z]{2})/[a-z]{2}/")

# Language preference: English first (Sierra Leone's legislative language), then French.
...
def _parse_profile(html_text: str) -> List[Tuple[str, str, Optional[str]]]:
    """Return [(detail_id, title, iso_date)] for every Sierra Leone legal text.
...
class WIPOLexSLScraper(BaseScraper):
    """Scraper for SL/WIPOLex."""
    ...
    def fetch_all(self) -> Generator[Dict[str, Any], None, None]:
        ...
        for detail_id, title, date in self._iter_docs():
            ...
            doc_id = f"SL_WIPOLEX_{detail_id}"
            text = None
            chosen_url = None
            language = None
            for url in urls:
    ...
    def fetch_updates(self, since: Any = None) -> Generator[Dict[str, Any], None, None]:
        # WIPO Lex has no incremental feed; re-scan the (small) SL corpus.
    ...
    def test(self) -> bool:
        logger.info("Testing SL/WIPOLex profile + detail + PDF extraction...")
        profile = _get(f"{HOST}{PROFILE_PATH}")
        if not profile:
            logger.error("Profile fetch failed")
            return False
        rows = _parse_profile(profile)
        logger.info("Parsed %d SL legal texts", len(rows))
        if len(rows) < 10:
            logger.error("Expected >=10 SL texts, got %d", len(rows))
            return False
